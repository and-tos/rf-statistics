...
def _intramolecular_protein_contact_df(
    los_contacts_df,
    ligand_atom_type,
    los_atom_ligand_atom_type,
    contact_angle_atoms,
    los_atom,
    central_atom,
    structure_analyzer,
    ...
    vdw_distance = (
        descriptors.MolecularDescriptors().atom_distance(central_atom, los_atom)
        - central_atom.vdw_radius
        - los_atom.vdw_radius
    )

    rf_dictionary_1 = structure_analyzer.total_rf.return_rf(
        los_atom, ligand_alphas, protein_alphas, ligand_atom_type, ligand_h, protein_h
    )

    rf_dictionary_2 = structure_analyzer.total_rf.return_rf(
        central_atom,
        protein_alphas,
        ligand_alphas,
        los_atom_ligand_atom_type,
    ...
    rf_ligands_errors = [
        rf_dictionary_1["rf_ligand_error"],
        rf_dictionary_2["rf_ligand_error"],
    ]
    rf_total = gmean(rf_ligands)
    rf_total_error = total_rf_calculator.error_propagation(
        rf_ligands, rf_ligands_errors
    )
...
class ProteinDescriptors(object):
    def __init__(
        ...
        :param rf_home: Path to RF lookup files, atom type definitions, Proasis database
        """
        ...
        self.protein_df = los_descriptors.rdkit_match_protein_atoms(
            self.rdkit_protein, self.csd_protein, self.protein_atom_types_df, pdb=pdb
        )
        self.pseudo_ligand_df = los_descriptors.match_ligand_atoms(
            self.rdkit_protein, self.protein_atoms
        )
    ...
    def return_protein_intramolecular_contact_df(self):
        ...
        # pseudo_ligand_atoms = utils.return_ligand_contacts(self.protein_atoms, [ligand_atoms], query_atom,
        #                                                            self.csd_protein, interaction_cutoff=0.5,
        #                                                            interatomic_cutoff=15)
        pseudo_ligand_atoms = [
            sorted(list(c.atoms), key=lambda x: x.label)
            for c in self.csd_protein.contacts(
                distance_range=(-5.0, 0.5), path_length_range=(-1, 999)
            )
            if c.is_in_line_of_sight
        ]
        pseudo_ligand_atoms = [
            contact_atoms
            for contact_atoms in pseudo_ligand_atoms
            if self.csd_protein.shortest_path(
                self.csd_protein.atom(contact_atoms[0].label),
                self.csd_protein.atom(contact_atoms[1].label),
            )
            == 0
            or self.csd_protein.shortest_path(
                self.csd_protein.atom(contact_atoms[0].label),
                self.csd_protein.atom(contact_atoms[1].label),
            )
            >= 4
        ]
        pseudo_ligand_atoms = set(
            [
                a[0]
                for a in pseudo_ligand_atoms
                if a[0].protein_atom_type != "ligand"
                and a[1].protein_atom_type != "ligand"
            ]
        )
        ...
        for central_atom in pseudo_ligand_atoms:
            ...
            if central_atom.protein_atom_type in [
                "Cofactor",
                "Ligand",
                "Nucleotide",
                "Metal",
                ...
                continue
            if (
                central_atom.partial_charge
                in self.pseudo_ligand_df["ligand_atom_index"].unique()
            ):
                los_contact_atoms = utils.return_los_contacts(
                    central_atom, pseudo_ligand_atoms
                )
                ligand_atom_type = self.pseudo_ligand_df[
                    self.pseudo_ligand_df["ligand_atom_index"]
                    == central_atom.partial_charge
                ]["ligand_atom_type"].to_numpy()[0]
                contact_angle_atoms = central_atom.neighbours
                for los_atom in los_contact_atoms:
                    if sorted(
                        (los_atom.partial_charge, central_atom.partial_charge)
                    ) in [
                        sorted(t)
                        for t in zip(
                            los_contacts_df["los_atom_index"],
                            los_contacts_df["ligand_atom_index"],
                        )
                    ]:
                        continue
                    if "_Z" in los_atom.label:
                        ...
                        continue
                    elif los_atom.protein_atom_type in [
                        "Cofactor",
                        "Ligand",
                        "Nucleotide",
                        "Metal",
                        ...
                        los_atom.los_at = self.protein_df[
                            self.protein_df["protein_atom_index"]
                            == los_atom.partial_charge
                    ...
                    los_atom.buriedness = utils.return_protein_buriedness(
                        ligand_atoms, los_atom, self.csd_protein
                    )
                    los_pair_dict = los_descriptors._contact_df(
                        ligand_atom_type,
                        contact_angle_atoms,
                        los_atom,
                        central_atom,
        ...
        # los_contacts_df = los_descriptors._return_primary_contacts(
        #     los_contacts_df, self.csd_protein
        # )
