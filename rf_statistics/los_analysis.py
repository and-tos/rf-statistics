...
def parse_args():
    ...
    parser.add_argument(
        "-i", "-in", "--input", help="Path to input folder.", default=os.getcwd()
    )

    parser.add_argument("--max_vdw_dist", help="", default="0.5", type=float)

    parser.add_argument("--min_alpha_i", help="", default="0", type=float)

    parser.add_argument("--max_alpha_i", help="", default="180", type=float)

    parser.add_argument(
        "-m", "--mode", help="ligand or protein mode", default="ligand", type=str
    )
    ...
    parser.add_argument("--n_boot", help="", default="100", type=int)

    parser.add_argument("-los", "--los", help="", default="los_filtered.csv")

    parser.add_argument(
        "-complex", "--complex", help="", default="complex_filtered.csv"
    )

    parser.add_argument(
        "-o", "-out", "--output", help="Path to output file.", default="rf.csv"
    )
...
def _calculate_hits_expected(
    ...
    """
    if randomize:
        acount_df = acount_df[acount_df["identifier"].isin(identifiers)].sample(
            len(identifiers), replace=True
        )
        scount_df = scount_df.iloc[acount_df.index]

    acount_df_filtered = acount_df.sum()
    scount_df_filtered = scount_df.sum()
    scount_df_filtered.index = scount_df_filtered.index.str.removeprefix("surf_area_")

    hits = acount_df_filtered[protein_atom_types].to_dict()
    expected = scount_df_filtered[protein_atom_types].to_dict()

    return hits, expected


def _reweigh_scount(totcount, scount_df):
    scount_df = scount_df.join(
        pd.DataFrame.from_dict(totcount, orient="index", columns=["totcount"]),
        on="identifier",
    )
    scount_df["totcount"] = scount_df["totcount"].infer_objects().fillna(0)
    columns = [c for c in scount_df.columns if c not in ["identifier", "totcount"]]
    scount_df.loc[:, columns] = scount_df.loc[:, columns].mul(
        scount_df["totcount"], axis=0
    )
...
class RfAtom(object):
    ...
    def __init__(
        ...
        second_geometry_max=None,
        second_geometry_min=None,
        ...
        Protein mode
        >>> complex_df = pd.DataFrame({'molecule_name': ['2P3T_010'], 'surf_area_other_ligand': [0], 'surf_area_other_central_ligand': [284.], 'surf_area_query_match': [15.],'surf_area_protein': [635]})
        ...
        if type(complex_input) == str or isinstance(complex_input, Path):
            self.complex_df = pd.read_csv(Path(self.input) / complex_input)
        else:
            self.complex_df = complex_input

        if type(los_input) == str or isinstance(los_input, Path):
            self.los_df = pd.read_csv(Path(self.input) / los_input)
        else:
    ...
    def _calculate_scount(self):
        ...
        scount_df = pd.DataFrame(
            columns=[
                f"surf_area_{protein_atom_type}" for protein_atom_type in self.a_list
            ]
            + ["identifier"]
        )  # interaction surfaces of los atoms per atom type. F(db_id)
        s_temp = {}
        ...
        for index, line in self.complex_df.iterrows():
            ...
            s_tot[db_id] = 0.0
            for protein_atom_type in self.surf_list:
            ...
            if self.watmap:
                s_temp[db_id] = line.loc[
                    f"surf_area_{self.surf_list[0]}" :f"surf_area_{self.surf_list[-2]}"
                ]
                s_temp[db_id] = s_temp[db_id].append(
                    line.loc["ligand_sasa":"ligand_sasa"]
                )
            else:
                s_temp[db_id] = line.loc[
                    [f"surf_area_{surf}" for surf in self.surf_list]
                ]
            _s = pd.concat(
                [s_temp[db_id].divide(s_tot[db_id]), (pd.Series({"identifier": db_id}))]
            )
            scount_df = pd.concat([scount_df, _s.to_frame().T], ignore_index=True)
    ...
    def _update_acount_and_refkey(self, db_id, line, refkey, db_hit, acount_df):
        ...
        if db_id != refkey:
            sum_row = (
                acount_df[acount_df["identifier"] == db_id].squeeze()[self.a_list]
                + line[self.a_list]
            )
            acount_df.loc[acount_df["identifier"] == db_id, self.a_list] = sum_row[
                self.a_list
            ].to_list()
    ...
    def run_los_multiprocessing(
        self, data, protein_atom_type, num_samples, acount_df, scount_df
    ):
        """
        ...
        calculate_hits_expected_parallel = partial(
            _calculate_hits_expected,
            identifiers=data,
        ...
        a_sum_ = [
            a[0][protein_atom_type] for a in out
        ]  # List with a_temp for each bootstrapping loop
        e_sum_ = [
            e[1][protein_atom_type] for e in out
        ]  # List with e_temp for each bootstrapping loop
    ...
    def run_single_processing(
        ...
        for n_sample in range(num_samples):
            a_temp, e_temp = _calculate_hits_expected(
                "",
                data,
                [protein_atom_type],
                acount_df,
    ...
    def _boots(self, data, protein_atom_type, num_samples, acount_df, scount_df):
        ...
        acount_df = acount_df.sort_values("identifier").reset_index(drop=True)
        scount_df = scount_df.sort_values("identifier").reset_index(drop=True)
        if num_samples >= 1000:
            print("Multiprocessing...")
            a_sum, e_sum = self.run_los_multiprocessing(
                data, protein_atom_type, num_samples, acount_df, scount_df
            )
        else:
            a_sum, e_sum = self.run_single_processing(
                data, protein_atom_type, num_samples, acount_df, scount_df
            )
    ...
    def _confidence_limits(
        self, data, protein_atom_type, num_samples, alpha, acount_df, scount_df
    ):
        rf_boots = self._boots(
            data, protein_atom_type, num_samples, acount_df, scount_df
        )
        rf_sorted = np.sort(rf_boots, axis=0)
        return (
            rf_sorted[int((alpha / 2.0) * num_samples)],
            rf_sorted[int((1 - alpha / 2.0) * num_samples)],
        )

    def _print_rf(
        self, protein_atom_type, e_full, a_full, db_list, acount_df, scount_df
    ):
        # a_full[protein_atom_type] is the number of contacts between the ligand query atom and the protein atom type
        rf = {}
        size = len(
            db_list[protein_atom_type].keys()
        )  # The number of occurrences of a protein atom type within all
        ...
        if e_full[protein_atom_type] == 0.0 or size == 0:
            ...
            _rf = a_full[protein_atom_type] / e_full[protein_atom_type]

            if (
                size <= 50 or _rf == "nan"
            ):  # consider minimum 50 entries to calculate Rf stats
                effect = "?"
                low = np.nan
                high = np.nan

            else:
                low, high = self._confidence_limits(
                    db_list[protein_atom_type].keys(),
                    protein_atom_type,
                    self.n_boot,
                    0.05,
    ...
    def _filter_los_input(self, los_input):
        ...
        los_input = los_input[
            (los_input["vdw_distance"] <= self.max_vdw_dist)
            | (self.watmap and los_input["res_name"] == "SOL")
        ]
        if not self.protein_perspective_water:
            los_input = los_input[
                (
                    (self.min_alpha_i <= los_input[self.angle_name])
                    & (los_input[self.angle_name] <= self.max_alpha_i)
                )
                | (self.watmap and los_input["res_name"] == "SOL")
            ]
        # drop duplicates if more than one angle within angle_range for the same contact atom
        los_input = los_input.drop_duplicates(
            subset=["res_name", "molecule_name", "distance", "query_atom_id"],
            keep="last",
        )
        if self.two_dimensions:
            ...
            los_input = los_input[
                (self.second_geometry_min <= abs(los_input[query_name]))
    ...
    def _get_rf_stats(self, e_full, a_full, db_list, acount_df, scount_df):
        ...
        for protein_atom_type in list_of_atom_types_to_calculate_rf:
            output_df = self._print_rf(
                protein_atom_type, e_full, a_full, db_list, acount_df, scount_df
            )
            output_df_all = pd.concat([output_df_all, output_df])

        if "alpha" in self.angle_name:
            output_df_all = output_df_all.assign(
                alpha_min=180 - self.max_alpha_i, alpha_max=180 - self.min_alpha_i
            )
        else:
            output_df_all[f"{self.angle_name}_max"] = self.max_alpha_i
            output_df_all[f"{self.angle_name}_min"] = self.min_alpha_i
        if self.second_geometry_name is not None:
            output_df_all[f"{self.second_geometry_name}_min"] = self.second_geometry_min
            output_df_all[f"{self.second_geometry_name}_max"] = self.second_geometry_max
        if self.export:
    ...
    def _get_rf_boots(self, db_list, acount_df, scount_df):
        ...
        if "alpha" in self.angle_name:
            output_df_all = output_df_all.assign(
                alpha_min=180 - self.max_alpha_i,
                alpha_max=180 - self.min_alpha_i,
        ...
        if self.second_geometry_name is not None:
            output_df_all = output_df_all.assign(
                h_min=self.second_geometry_min, h_max=self.second_geometry_max
            )
    ...
    def _count_los(self, los_input, blacklist, scount_df):
        acount_df = pd.DataFrame(
            0.0, columns=self.a_list + ["identifier"], index=scount_df.index
        )
        ...
        if acount_df_sum.shape[0]:
            acount_df_sum = (
                acount_df_sum.groupby("identifier")["los_atom_type"]
                .value_counts()
                .unstack()
                .drop(columns=["0.0", "same_ligand"], errors="ignore")
                .reset_index()
            )
        ...
        acount_df = (
            pd.concat(
                [
                    acount_df_sum,
                    acount_df[
                        acount_df["identifier"].isin(acount_df_sum["identifier"])
                        == False
                    ],
                ],
                ignore_index=True,
    ...
    def calculate_acount_scount(self):
        ...
        if self.second_geometry_name is not None:
            los_df[f"{self.second_geometry_name}"] = los_df[
    ...
    def calculate_rf(self):
        ...
        a_full, e_full = _calculate_hits_expected(
            "", db_all, self.a_list, acount_df, scount_df
        )
...
def main():
    args = parse_args()
    los_a = RfAtom(
        second_geometry_name=args.second_geometry_name,
        second_geometry_max=args.second_geometry_max,
        second_geometry_min=args.second_geometry_min,
        complex_input=args.complex,
        input_path=args.input,
        los_input=args.los,
        max_alpha_i=args.max_alpha_i,
        min_alpha_i=args.min_alpha_i,
        max_vdw_dist=args.max_vdw_dist,
        n_boot=args.n_boot,
        no_export=args.no_export,
        output_path=args.output,
        watmap=args.watmap,
        protein_atom_types=args.protein_atom_types,
        mode=args.mode,
    )
