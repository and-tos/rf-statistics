...
from rf_statistics import utils, rf_structure_analysis
...
def return_assignation_df(rdkit_ligand, ligand_atom_types_df, atoms_df):
    ...
    for index, row in ligand_atom_types_df.iterrows():
        ...
        matches = rdkit_ligand.GetSubstructMatches(
            substructure, maxMatches=100000, uniquify=False
        )
        if len(matches) > 0:
            matches = [
                match[smarts_index]
                ...
                not in assignation_df["ligand_atom_index"].to_list()
            ]
            atom_indices = atoms_df[atoms_df["atom_index"].isin(matches)][
                "atom_partial_charge"
            ].to_list()
            atom_labels = atoms_df[atoms_df["atom_index"].isin(matches)][
                "atom_label"
            ].to_list()
            ...
            df = pd.DataFrame(
                {
                    "ligand_atom_index": atom_indices,
                    "ligand_atom_label": atom_labels,
                    "ligand_atom_type": ligand_atom_types,
                    "pi_atom": pi_atoms,
                }
            )
            assignation_df = pd.concat([assignation_df, df], ignore_index=True)
...
def match_ligand_atoms(rdkit_ligand, csd_ligand_atoms, freq_threshold=1500):
    ...
    >>> assignation_df.shapeunassigned_df
    (29, 4)
    """
    ...
    frequent_ligand_atom_types_df = ligand_atom_types_df[
        ligand_atom_types_df["combined_occurrences_filtered"] >= freq_threshold
    ]
    assignation_df = return_assignation_df(
        rdkit_ligand, frequent_ligand_atom_types_df, atoms_df
    )
    ...
    if len(assignation_df["ligand_atom_index"]) != rdkit_ligand.GetNumAtoms():
        ...
        complementary_assignation_df = return_assignation_df(
            rdkit_ligand, infrequent_ligand_atom_types_df, atoms_df
        )
        complementary_assignation_df = complementary_assignation_df[
            ~complementary_assignation_df.ligand_atom_index.isin(
                assignation_df["ligand_atom_index"]
            )
        ]
        assignation_df = pd.concat(
            [assignation_df, complementary_assignation_df], ignore_index=True
        )

    unassigned_df = atoms_df[
        (atoms_df["atom_label"].str.startswith("_"))
        & (atoms_df["atom_label"].isin(assignation_df["ligand_atom_label"]) == False)
    ]
    if unassigned_df.shape[0] > 0:
        unassigned_df = unassigned_df.rename(
            columns={
                "atom_label": "ligand_atom_label",
                "atom_index": "ligand_atom_index",
            }
        )
        unassigned_df = unassigned_df[["ligand_atom_label", "ligand_atom_index"]]
        unassigned_df["pi_atom"] = 0
        unassigned_df["ligand_atom_type"] = "unassigned"
        assignation_df = pd.concat([assignation_df, unassigned_df], ignore_index=True)
    return assignation_df


def rdkit_match_protein_atoms(
    rdkit_protein,
    ccdc_protein,
    ...
    ccdc_protein_atoms = ccdc_protein.atoms
    atoms_df = pd.DataFrame.from_records(
        [
            (at.label, int(at.partial_charge), cnt)
            for cnt, at in enumerate(ccdc_protein_atoms)
        ],
        columns=["atom_label", "atom_partial_charge", "atom_index"],
    )
    ...
    for index, row in protein_atom_types_df.iterrows():
        ...
        substructure.UpdatePropertyCache()
        Chem.rdmolops.FastFindRings(substructure)
        matches = rdkit_protein.GetSubstructMatches(
            substructure, maxMatches=100000, uniquify=False
        )
        ...
        if len(matches) > 0:
            ...
            if pdb:
                if protein_atom_type == "Water":
                    matches = [
                        ...
                        for match in matches
                        if match[smarts_index]
                        not in assignation_df["protein_atom_index"].to_list()
                    ]
                else:
                    matches = [
                        match[smarts_index]
                        ...
                        if match[smarts_index]
                        not in assignation_df["protein_atom_index"].to_list()
                        and not rdkit_protein.GetAtomWithIdx(match[smarts_index])
                        .GetPDBResidueInfo()
                        .GetIsHeteroAtom()
                    ]
            else:
                matches = [
                    ...
                    not in assignation_df["protein_atom_index"].to_list()
                    and "_Z"
                    not in rdkit_protein.GetAtomWithIdx(match[smarts_index]).GetProp(
                        "_TriposAtomName"
                    )
                ]
            if index_is_in_atom_name:
                matches = [
                    rdkit_protein.GetAtomWithIdx(match_index)
                    .GetPDBResidueInfo()
                    .GetName()
                    .strip()
            ...
            contact_dict["protein_atom_index"] = (
                contact_dict["protein_atom_index"] + atom_indices
            )
            contact_dict["protein_atom_label"] = (
                contact_dict["protein_atom_label"] + atom_labels
            )
            contact_dict["protein_atom_type"] = (
                contact_dict["protein_atom_type"] + protein_atom_types
            )
    ...
    if len(assignation_df["protein_atom_index"]) != rdkit_protein.GetNumAtoms():
        ...
        unassigned_indices = [
            i for i in range(rdkit_protein.GetNumAtoms()) if i not in assigned_indices
        ]
        for i in unassigned_indices:
            if is_metal(rdkit_protein.GetAtomWithIdx(i)):
                df = pd.DataFrame(
                    {
                        "protein_atom_index": [i],
                        "protein_atom_label": atoms_df[atoms_df["atom_index"] == i][
                            "atom_label"
                        ].to_list(),
    ...
    # disulfides
    cys_sulfur_atoms = [a for a in ccdc_protein.atoms if a.label.startswith("_CS")]
    for a in cys_sulfur_atoms:
        if utils.sulfur_is_part_of_disulfide(a, ccdc_protein):
            assignation_df.loc[
                assignation_df["protein_atom_label"] == a.label, "protein_atom_type"
            ] = "S_apol"
...
def match_protein_atoms(ccdc_protein, protein_atom_types_df):
    ...
    for atomic_symbol in protein_elements:
        protein_atom_types = protein_atom_types_df[
            protein_atom_types_df["atomic_symbol"] == atomic_symbol
        ]
        atoms_to_match = [
            at
            for at in unmatched_atoms
            if at.atomic_symbol == atomic_symbol
            and at.protein_atom_type == "Amino_acid"
            or at.protein_atom_type == "Water"
            and "_Z" not in at.label
        ]
        substructures = [
            search.SMARTSSubstructure(pat_smarts)
            for pat_smarts in protein_atom_types["CCDC_SMARTS"].values
        ]
    ...
    for at in [
        at
        for at in unmatched_atoms
        if at.atomic_symbol == "S" and at.protein_atom_type == "Amino_acid"
    ]:  # this is to match sulfur which could be covalently bound
        protein_atom_type = utils.return_los_pat(
            at, ccdc_protein, protein_atom_types_df
        )
        pat_dict["protein_atom_index"].append(at.index)
    ...
    #         protein_atom_type = utils.return_los_pat(at, ccdc_protein, protein_atom_types_df)
    #         pat_dict['protein_atom_index'].append(at.index)
    #         pat_dict['protein_atom_type'].append(protein_atom_type)
...
def _geometries_match(
    ...
    if (
        ligand_h != ligand_h
        or ligand_h is None
        or ligand_atom_type_df[f"{interaction_type}_h_min"].to_numpy()[0]
        <= ligand_h
        <= ligand_atom_type_df[f"{interaction_type}_h_max"].to_numpy()[0]
    ):
        if (
            protein_h != protein_h
            or protein_h is None
            or protein_atom_type_df[f"{interaction_type}_h_min"].to_numpy()[0]
            <= protein_h
            <= protein_atom_type_df[f"{interaction_type}_h_max"].to_numpy()[0]
        ):
            for ligand_alpha in ligand_alphas:
                if (
                    ligand_atom_type_df[f"{interaction_type}_alpha_min"].to_numpy()[0]
                    <= ligand_alpha
                    <= ligand_atom_type_df[f"{interaction_type}_alpha_max"].to_numpy()[
                        0
                    ]
                ):
                    for protein_alpha in protein_alphas:
                        if (
                            protein_atom_type_df[
                                f"{interaction_type}_alpha_min"
                            ].to_numpy()[0]
                            <= protein_alpha
                            <= protein_atom_type_df[
                                f"{interaction_type}_alpha_max"
                            ].to_numpy()[0]
                        ):
                            return True
    return False
...
def return_interaction_type(
    ...
    protein_atom_type_df = pd.read_csv(
        Path(rf_home) / "protein_atom_types.csv", sep="\t"
    )
    protein_atom_type_df = protein_atom_type_df.loc[
        protein_atom_type_df["protein_atom_type"] == protein_atom_type
    ]
    ligand_atom_type_df = pd.read_csv(Path(rf_home) / "ligand_atom_types.csv", sep="\t")
    ligand_atom_type_df = ligand_atom_type_df.loc[
        ligand_atom_type_df["ligand_atom_type"] == ligand_atom_type
    ]

    try:
        if type(ligand_alphas) == str:
            ligand_alphas = [
                float(ligand_alpha) for ligand_alpha in ligand_alphas.split(";")
            ]
        if type(protein_alphas) == str:
            protein_alphas = [
                float(protein_alpha) for protein_alpha in protein_alphas.split(";")
            ]
        ...
        if protein_atom_type == "other_ligand":
            ...
            if (
                ligand_atom_type_df["halogen_don"].to_numpy()[0] == 1
                and protein_atom_type_df["halogen_acc"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
                    interaction_type,
                    ligand_alphas,
            ...
            if (
                ligand_atom_type_df["halogen_acc"].to_numpy()[0] == 1
                and protein_atom_type_df["halogen_don"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
            ...
            if (
                ligand_atom_type_df["ionic_pos"].to_numpy()[0] == 1
                and protein_atom_type_df["ionic_neg"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
            ...
            if (
                ligand_atom_type_df["ionic_neg"].to_numpy()[0] == 1
                and protein_atom_type_df["ionic_pos"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
            ...
            if (
                ligand_atom_type_df["hbond_acc"].to_numpy()[0] == 1
                and protein_atom_type_df["hbond_don"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
                    ...
                    if (
                        ligand_atom_type_df["hbond_classic"].to_numpy()[0] == 1
                        and protein_atom_type_df["hbond_classic"].to_numpy()[0] == 1
                    ):
            ...
            if (
                ligand_atom_type_df["hbond_don"].to_numpy()[0] == 1
                and protein_atom_type_df["hbond_acc"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
            ...
            if (
                ligand_atom_type_df["multipolar_pos"].to_numpy()[0] == 1
                and protein_atom_type_df["multipolar_neg"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
            ...
            if (
                ligand_atom_type_df["multipolar_neg"].to_numpy()[0] == 1
                and protein_atom_type_df["multipolar_pos"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
            ...
            interaction_type = "pi"
            if (
                ligand_atom_type_df["pi"].to_numpy()[0] == 1
                and protein_atom_type_df["pi"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
            ...
            if (
                ligand_atom_type_df["hydrophobic"].to_numpy()[0] == 1
                and protein_atom_type_df["hydrophobic"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
            ...
            if (
                ligand_atom_type_df["hydrophobic"].to_numpy()[0] == 1
                and protein_atom_type_df["hydrophobic"].to_numpy()[0] == 0
            ):
                if _geometries_match(
                    ligand_atom_type_df,
                    protein_atom_type_df,
            ...
            if (
                ligand_atom_type_df["hydrophobic"].to_numpy()[0] == 0
                and protein_atom_type_df["hydrophobic"].to_numpy()[0] == 1
            ):
                if _geometries_match(
                    ligand_atom_type_df,
...
def get_primary_contacts_df(df, ccdc_protein, ligand_atom_label):
    assigned_dfs = []
    while df.shape[0] > 0:
        central_atom = ccdc_protein.atom(ligand_atom_label)
        shortest_los_atom_label = df.iloc[0]["los_atom_label"]
        df.loc[df.index[0], "is_primary"] = True
        df.loc[df.index[0], "primary_los_atom"] = shortest_los_atom_label
        shortest_los_atom = ccdc_protein.atom(shortest_los_atom_label)
        for index, row in df.iterrows():
            if row["los_atom_label"] == shortest_los_atom_label:
                continue
            los_atom2 = ccdc_protein.atom(row["los_atom_label"])
            angle = descriptors.MolecularDescriptors.atom_angle(
                shortest_los_atom, central_atom, los_atom2
            )
            if angle < 45:
                df.loc[index, "is_primary"] = False
                df.loc[index, "primary_los_atom"] = shortest_los_atom_label
        assigned_dfs.append(df[df["is_primary"].isna() == False])
        df = df[df["is_primary"].isna()]

    assigned_df = pd.concat(assigned_dfs)
    assigned_df["rf_total_clash_corrected"] = assigned_df["rf_total"]
    assigned_df.loc[assigned_df["is_clash"], "rf_total_clash_corrected"] = 0.1
    assigned_df["primary_rf"] = assigned_df.groupby("primary_los_atom")[
        "rf_total_clash_corrected"
    ].transform(gmean)
    assigned_df["atom_rf"] = assigned_df.drop_duplicates("primary_los_atom")[
        "primary_rf"
    ].product()
    return assigned_df


def return_primary_contacts_df(contacts_df, ccdc_protein):
    contacts_df = contacts_df.sort_values("vdw_distance", ascending=True)
    primary_contacts_dfs = []
    for ligand_atom_label, gdf in contacts_df.groupby("ligand_atom_label"):
        primary_contacts_dfs.append(
            get_primary_contacts_df(gdf, ccdc_protein, ligand_atom_label)
        )
    contacts_df = pd.concat(primary_contacts_dfs)
    return contacts_df


# deprecated
# def _is_primary(index, ccdc_protein, contact_atom1, central_atom, df, mode="ligand"):
#     is_primary = True
#     primary_atom = contact_atom1

#     for index2, row2 in df.iterrows():
#         if index2 == index:
#             continue
#         if mode == "ligand":
#             contact_atom2 = ccdc_protein.atom(row2["los_atom_label"])
#         else:
#             contact_atom2 = ccdc_protein.atom(row2["ligand_atom_label"])
#         angle = descriptors.MolecularDescriptors.atom_angle(
#             contact_atom1, central_atom, contact_atom2
#         )
#         if angle < 25:
#             is_primary = False
#             return False, contact_atom1
#         else:
#             primary_atom = contact_atom2
#     return True, contact_atom2


# def _return_primary_contacts(entry_contact_df, ccdc_protein):
...
#         central_atom = ccdc_protein.atom(ligand_atom_label)
#         df = entry_contact_df.loc[ligand_atom_labels == ligand_atom_label].reset_index(
#             drop=False
#         )
#         for index, row in df.iterrows():
#             contact_atom1 = ccdc_protein.atom(row["los_atom_label"])
#             (
#                 entry_contact_df.loc[row["index"], "is_primary"],
#                 primary_atom,
#             ) = _is_primary(
#                 index, ccdc_protein, contact_atom1, central_atom, df[index + 1 :]
#             )
#             entry_contact_df.loc[row["index"], "primary_atom"] = primary_atom
...
#         central_atom = ccdc_protein.atom(los_atom_label)
#         df = filtered_df.loc[los_atom_labels == los_atom_label].reset_index(drop=False)
#         for index, row in df.iterrows():
#             contact_atom1 = ccdc_protein.atom(row["ligand_atom_label"])
#             entry_contact_df.loc[row["index"], "is_primary"] = _is_primary(
#                 index,
#                 ccdc_protein,
#                 contact_atom1,
#                 central_atom,
...
def _contact_df(
    ...
    for contact_angle_atom in contact_angle_atoms:
        ligand_alpha_i = descriptors.GeometricDescriptors.point_angle(
            contact_angle_atom.coordinates,
    ...
    for neighbour in los_atom.neighbours:
        protein_alpha_i = descriptors.GeometricDescriptors.point_angle(
            neighbour.coordinates, los_atom.coordinates, central_atom.coordinates
        )
        protein_alphas.append(180 - protein_alpha_i)

    ligand_h = structure_analyzer.return_ligand_h(
        ligand_atom_type, los_atom, central_atom
    )
    ...
    vdw_distance = distance - central_atom.vdw_radius - los_atom.vdw_radius

    rf_dictionary = structure_analyzer.total_rf.return_rf(
        los_atom,
        ligand_alphas,
        protein_alphas,
        distance,
    ...
    los_pair_dic = {
        ...
        "los_atom_chain": los_atom.chain_label,
        "ligand_atom_chain": central_atom.chain_label,
...
def binned_rf_contacts(rf_total):
    bins = [0.0, 0.1, 0.3, 0.5, 0.8, 1.0, 1.2, 1.5, 2.0, 3.0, 5.0, 8.0]
    binned_contacts = (
        pd.cut(rf_total, bins)
        .value_counts()
        .to_frame()
        .sort_index()
        .transpose()
...
def get_atom_rf_count_df(contact_df):
    contact_df = contact_df.drop_duplicates("ligand_atom_label")
    binned_atom_rf = binned_rf_contacts(contact_df["atom_rf"])
    binned_atom_rf = binned_atom_rf.rename(
        columns={c: f"atom_rf_{c}" for c in binned_atom_rf.columns}
    )
    contact_df.loc[:, "atom_rf"] = contact_df["atom_rf"].replace(0.0, 0.1)
    binned_atom_rf["atom_rf_gmean"] = gmean(contact_df["atom_rf"])
    binned_atom_rf["atom_rf_prod"] = contact_df["atom_rf"].product()
    return binned_atom_rf
...
def rf_count_df(contact_df, csd_ligand):
    ...
    non_competitive_contact_count = non_competitive_contact_count[
        non_competitive_contact_count < 1.0
    ].count()
    competitive_contact_count = rf_total - rf_total_error
    competitive_contact_count = competitive_contact_count[
        competitive_contact_count > 1.0
    ].count()
    ...
    non_competitive_contact_count_d05 = non_competitive_contact_count_d05[
        non_competitive_contact_count_d05 < 0.5
    ].count()
    competitive_contact_count_d05 = rf_total - rf_total_error
    competitive_contact_count_d05 = competitive_contact_count_d05[
        competitive_contact_count_d05 > 1.5
    ].count()

    heavy_atom_count = len(csd_ligand.atoms)
    rf_max = rf_total.max()
    rf_min = rf_total.min()
    clash_count_non_hbond = contact_df[
        (contact_df["is_intramolecular"] == False)
        & (contact_df["interaction_type"] != "hbond_classic")
    ]["vdw_distance"][contact_df["vdw_distance"] < -0.5].count()
    clash_count_hbond = contact_df[
        (contact_df["is_intramolecular"] == False)
        & (contact_df["interaction_type"] == "hbond_classic")
    ]["vdw_distance"][contact_df["vdw_distance"] < -0.7].count()
    ...
    rf_count_df = pd.DataFrame(
        {
            "non_competitive_contact_count": [non_competitive_contact_count],
            "competitive_contact_count": [competitive_contact_count],
            "non_competitive_contacts_per_atom": [
                non_competitive_contact_count / heavy_atom_count
            ],
            "competitive_contacts_per_atom": [
                competitive_contact_count / heavy_atom_count
            ],
            "non_competitive_contact_count_d01": [non_competitive_contact_count_d01],
            "competitive_contact_count_d01": [competitive_contact_count_d01],
            ...
            "competitive_contacts_per_atom_d01": [
                competitive_contact_count_d01 / heavy_atom_count
            ],
            "non_competitive_contact_count_d03": [non_competitive_contact_count_d03],
            "competitive_contact_count_d03": [competitive_contact_count_d03],
            ...
            "competitive_contacts_per_atom_d03": [
                competitive_contact_count_d03 / heavy_atom_count
            ],
            "non_competitive_contact_count_d05": [non_competitive_contact_count_d05],
            "competitive_contact_count_d05": [competitive_contact_count_d05],
            ...
            "competitive_contacts_per_atom_d05": [
                competitive_contact_count_d05 / heavy_atom_count
            ],
            "identifier": [identifier],
...
def get_hbond_df(mol, intermolecular_only: bool = True) -> pd.DataFrame:
    ...
    if intermolecular_only:
        ...
        hbonds = mol.hbonds(
            path_length_range=(-1, 999),
            distance_range=(-5, 0.2),
            hbond_criterion=hbond_criterion,
        )
    else:
        hbonds = mol.hbonds(
            path_length_range=(4, 999),
            distance_range=(-5, 0.2),
            hbond_criterion=hbond_criterion,
        )
    atom1_labels = []
    ...
    hbond_df = pd.DataFrame(
        {
            "atom1_label": atom1_labels,
            "atom2_label": atom2_labels,
            "atom1_residue_label": atom1_res_labels,
            "atom2_residue_label": atom2_res_labels,
            "h_atom_label": h_atom_labels,
            "distance": distances,
        }
    )
    hbond_df = hbond_df.sort_values("distance", ascending=True).drop_duplicates(
        "h_atom_label"
    )
    return hbond_df
...
def get_hbond_mismatch_df(csd_protein, interaction_cutoff):
    ...
    for at in central_ligand_atoms:
        at.ligand_index = -1
    close_contact_atoms = utils.return_ligand_contacts(
        [central_ligand_atoms],
        central_ligand_atoms[0],
        csd_protein,
    ...
    for central_atom in central_ligand_atoms:
        los_contact_atoms = utils.return_los_contacts(
            central_atom,
            list(close_contact_atoms),
            interaction_cutoff=interaction_cutoff,
        )
        if central_atom.is_acceptor:
            ...
            for acceptor_los_atom in acceptor_los_atoms:
                ...
                hbond_mismatches["atom1_residue_label"].append(
                    central_atom.residue_label
                )
                hbond_mismatches["atom2_label"].append(acceptor_los_atom.label)
                hbond_mismatches["atom2_residue_label"].append(
                    acceptor_los_atom.residue_label
                )
            ...
            if central_heavy_atom.is_donor:
                ...
                for h_los_atom in h_los_atoms:
                    if not h_los_atom.is_in_line_of_sight(central_heavy_atom):
                        donor_heavy_atoms = [
                            a.neighbours[0] for a in h_los_atoms if a.is_donor
                        ]
                        for donor_heavy_atom in donor_heavy_atoms:
                            hbond_mismatches["atom1_label"].append(
                                central_heavy_atom.label
                            )
                            hbond_mismatches["atom1_residue_label"].append(
                                central_heavy_atom.residue_label
                            )
                            hbond_mismatches["atom2_label"].append(
                                donor_heavy_atom.label
                            )
                            hbond_mismatches["atom2_residue_label"].append(
                                donor_heavy_atom.residue_label
                            )
    hbond_mismatch_df = pd.DataFrame(hbond_mismatches)
    return hbond_mismatch_df


def torsion_df(input_ligand):
    file_name = "".join(random.choice(string.ascii_lowercase) for i in range(8))
    sp.call(
        f"torsionchecker -t torlib/TorLib.xml -d 0 -m {input_ligand} -r torsion_check_{file_name}",
        shell=True,
        stdout=sp.DEVNULL,
        stderr=sp.STDOUT,
    )
    torsion_df = (
        pd.read_csv(f"torsion_check_{file_name}.csv", sep="\t")
        .drop(["Name"], axis=1)
        .reset_index(drop=True)
    )
    Path(f"torsion_check_{file_name}.csv").unlink()
...
def _is_clash(atom1, atom2):
    if (
        descriptors.MolecularDescriptors.atom_distance(atom1, atom2)
        - atom1.vdw_radius
        - atom2.vdw_radius
        < -0.5
    ):
...
def _contact_df_loop(
    ...
    protein,
    identifier,
    csd_ligand_entry=None,
    ligand_file=None,
    interaction_cutoff=0.5,
):
    los_contacts_dict_list = []
    ligand_atom_labels = ligand_df["ligand_atom_label"].to_list()

    for central_atom in central_ligand_atoms:
        if central_atom.label in ligand_atom_labels:
            los_contact_atoms = utils.return_los_contacts(
                central_atom,
                list(close_contact_atoms),
                interaction_cutoff=interaction_cutoff,
            )
            los_contact_atoms.extend(
                [a for a in list(close_contact_atoms) if _is_clash(central_atom, a)]
            )
            los_contact_atoms = list(set(los_contact_atoms))
            ligand_atom_type = ligand_df[
                ligand_df["ligand_atom_label"] == central_atom.label
            ...
            for los_atom in los_contact_atoms:
                if los_atom in central_ligand_atoms or los_atom.atomic_symbol == "H":
                    continue
                los_atom.los_at = protein_df[
                    (protein_df["protein_atom_label"] == los_atom.label)
                    & (protein_df["protein_atom_index"] == los_atom.partial_charge)
                ]["protein_atom_type"].to_numpy()[0]
                los_atom.buriedness = utils.return_protein_buriedness(
                    central_ligand_atoms, los_atom, protein
                )
                los_pair_dict = _contact_df(
                    ligand_atom_type,
                    contact_angle_atoms,
                    los_atom,
                    central_atom,
...
def get_b_factors(pdb_protein_file):
    ...
    for atom in pdb_protein.heavy_atoms:
        if not hasattr(atom.displacement_parameters, "temperature_factor"):
            bfactors.append(0)
        else:
...
def get_contacts_df(ccdc_protein):
    contacts = ccdc_protein.contacts(
        distance_range=(-5, 0.5), path_length_range=(-1, 999)
    )
    los_contacts = [c for c in contacts if contacts.is_in_line_of_sight]
    return
...
class RfDescriptors(object):
    def __init__(
        ...
        self.ligand_atom_types_df = pd.read_csv(
            atom_type_path / "ligand_atom_types.csv", sep="\t"
        )

        self.csd_protein.kekulize()
        self.csd_protein.remove_hydrogens()
        self.rdkit_ligand = Chem.MolFromMol2Block(
            self.csd_ligand.to_string(), removeHs=False
        )
        self.rdkit_protein = utils.get_rdkit_protein_from_csd_protein(self.csd_protein)

        self.rdkit_ligand.UpdatePropertyCache()
        Chem.rdmolops.FastFindRings(self.rdkit_ligand)

        self.csd_protein = utils.assign_index_to_atom_partial_charge(self.csd_protein)
        protein_df = rdkit_match_protein_atoms(
            self.rdkit_protein, self.csd_protein, self.protein_atom_types_df
        )

        # assign het groups
        protein_df.loc[
            (protein_df["protein_atom_label"].str.startswith("_U"))
            & (
                protein_df["protein_atom_type"].isin(["other_ligand", "metal"]) == False
            ),
            "protein_atom_type",
        ] = "other_ligand"
        ...
        self.protein_df = protein_df[
            protein_df["protein_atom_label"].isin(self.ligand_df["ligand_atom_label"])
            == False
        ]
        ...
        central_ligand_atoms = [
            self.csd_protein.atom(at.label) for at in self.csd_ligand.heavy_atoms
        ]
        ligand_smiles = self.csd_ligand.smiles
        for at in central_ligand_atoms:
            at.ligand_index = utils.ligand_index([central_ligand_atoms], at)
        close_contact_atoms = utils.return_ligand_contacts(
            [central_ligand_atoms], central_ligand_atoms[0], self.csd_protein
        )
        self.los_contacts_df = pd.DataFrame()
        if close_contact_atoms:
            los_contacts_df = _contact_df_loop(
                central_ligand_atoms,
                self.ligand_df,
                close_contact_atoms,
                self.protein_df,
        ...
        if calculate_surface:
            self.get_surface_df()
            if self.atom_surface_area_df.shape[0] == 0:
                self.los_contacts_df = pd.DataFrame()
        if intramolecular_contacts:

            pro_desc = ProteinDescriptors(
                self.csd_protein, self.csd_ligand, self.rdkit_protein
            )
            protein_contacts_df = pro_desc.return_protein_intramolecular_contact_df()
            self.los_contacts_df = pd.concat(
                [self.los_contacts_df, protein_contacts_df], ignore_index=True
            )
        return

    def get_surface_df(self):
        ligand_surf_dict = {}
        bs_surf_dict = {}

        ligand_atoms = [
            a for a in self.csd_protein.heavy_atoms if a.label.startswith("_Z")
        ]

        holo_protein = self.csd_protein.copy()
        holo_protein.remove_atoms(
            [a for a in holo_protein.heavy_atoms if a.label.startswith("_Z")]
        )
        ...
        for ligand_atom in ligand_atoms:
            ats = searcher.atoms_within_range(ligand_atom.coordinates, 4.3)
            for contact_atom in ats:
                if contact_atom.label.startswith("_Z"):
                    continue
                distance = descriptors.MolecularDescriptors.atom_distance(
                    ligand_atom, contact_atom
                )
                if distance < 2.25:
                    covalent_pairs.append((contact_atom, ligand_atom))

                if contact_atom not in bs_atoms:
                    if (
                        distance - ligand_atom.vdw_radius - contact_atom.vdw_radius
                        <= 0.5
                    ):
                        bs_atoms.append(contact_atom)

        atom_surface_area_df = self.protein_df.copy()

        warhead_atoms = []
        protein_atoms = self.csd_protein.atoms
        for a1, a2 in covalent_pairs:
            a1 = self.csd_protein.atom(a1.label)
            a2 = self.csd_protein.atom(a2.label)
            warhead_atoms.extend(
                [
                    a
                    for a in protein_atoms
                    if 0 < self.csd_protein.shortest_path(a, a1) < 3
                ]
            )
            warhead_atoms.extend(
                [
                    a
                    for a in protein_atoms
                    if 0 < self.csd_protein.shortest_path(a, a2) < 3
                ]
            )
            warhead_atoms.extend([a1, a2])
        warhead_atoms = list(set(warhead_atoms))
        warhead_atom_labels = [a.label for a in warhead_atoms]

        for bs_atom in bs_atoms:
            if bs_atom in warhead_atoms:
                continue

            atom_surf = utils.get_atom_surface(
                bs_atom,
                probe_radius=0,
                ligand_atoms=ligand_atoms,
                filter_by_ligand=True,
            )
            atom_surface_area_df.loc[
                self.protein_df["protein_atom_label"] == bs_atom.label,
                "protein_atom_surface",
            ] = atom_surf
            bs_surf_dict[bs_atom.label] = atom_surf

        if not "protein_atom_surface" in atom_surface_area_df.columns:
            self.atom_surface_area_df = pd.DataFrame()
            return

        atom_surface_area_df = atom_surface_area_df[
            atom_surface_area_df["protein_atom_surface"].isna() == False
        ][["protein_atom_label", "protein_atom_surface", "protein_atom_type"]]
        self.atom_surface_area_df = atom_surface_area_df.rename(
            columns={
                "protein_atom_label": "atom_label",
                "protein_atom_surface": "atom_surface",
            }
        )
        ...
        for ligand_atom in ligand_atoms:
            atom_surf = utils.get_atom_surface(
                ligand_atom,
                probe_radius=0,
                ligand_atoms=bs_atoms,
                filter_by_ligand=True,
            )
            ligand_surface += atom_surf
            ligand_surf_dict[ligand_atom.label] = atom_surf

        surf_dict = {"atom_label": [], "atom_surface": []}
        for ligand_atom_label in ligand_surf_dict:
            self.los_contacts_df.loc[
                self.los_contacts_df["ligand_atom_label"] == ligand_atom_label,
                "ligand_atom_surf",
            ] = ligand_surf_dict[ligand_atom_label]
            surf_dict["atom_label"].append(ligand_atom_label)
            surf_dict["atom_surface"].append(ligand_surf_dict[ligand_atom_label])

        self.atom_surface_area_df = pd.concat(
            [self.atom_surface_area_df, pd.DataFrame(surf_dict)], ignore_index=True
        )
        self.atom_surface_area_df["molecule_name"] = self.csd_protein.identifier
        self.atom_surface_area_df["atom_is_warhead"] = self.atom_surface_area_df[
            "atom_label"
        ].isin(warhead_atom_labels)

        for bs_atom_label in bs_surf_dict:
            self.los_contacts_df.loc[
                self.los_contacts_df["los_atom_label"] == bs_atom_label, "los_atom_surf"
            ] = bs_surf_dict[bs_atom_label]

        self.los_contacts_df["ligand_surface"] = ligand_surface

        self.los_contacts_df["los_atom_is_warhead"] = self.los_contacts_df[
            "los_atom_label"
        ].isin(warhead_atom_labels)
        self.los_contacts_df["ligand_atom_is_warhead"] = self.los_contacts_df[
            "ligand_atom_label"
        ].isin(warhead_atom_labels)
...
class CsdDescriptorsFromMol2(object):
    def __init__(self, input_protein):
        ...
        self.protein = utils.assign_index_to_atom_partial_charge(self.protein)
...
class CsdDescriptorsFromProasis(object):
    ...
    def __init__(
        ...
        protein_df = describer.protein_df
        self.protein_df = protein_df

        if calculate_surface:
            self.atom_surface_area_df = describer.atom_surface_area_df
        else:
            self.atom_surface_area_df = pd.DataFrame()
...
class CsdDescriptorsFromGold(object):
    def __init__(
        ...
        self.protein.remove_ligand(self.protein.ligands[-1].identifier)
        self.protein.add_ligand(self.csd_ligand_entry.molecule)
        ...
        if "template_strucid" in self.csd_ligand_entry.attributes:
            self.protein.identifier = self.csd_ligand_entry.attributes[
                "template_strucid"
            ]
        else:
            self.protein.identifier = str(
                Path(self.input_ligand).absolute().parent
            ).split("_")[-2]

        if only_binding_site:
            self.protein = _cut_out_binding_site_by_residue_df(
                self.protein,
                str(Path(self.bfactor) / Path("binding_site_residues.csv")),
            )
        ...
        if self.bfactor and pdb_file:
            ...
            docked_water_residue_labels = [
                i.heavy_atoms[0].residue_label
                for i in docking_results.ligands[0].docked_waters
            ]
            docked_water_indices = [
                int(i.split("HOH_AM")[1]) - 1 for i in docked_water_residue_labels
            ]
            water_cnt = 0
            for atom in self.protein.atoms:
                if (
                    atom.protein_atom_type == "Water"
                    and atom.atomic_symbol != "H"
                    and docked_water_residue_labels
                ):
                    docked_water_index = docked_water_indices[water_cnt]
                    atom_label = (
                        docking_settings.waters[docked_water_index].heavy_atoms[0].label
                    )
                    atom_residue_label = (
                        docking_settings.waters[docked_water_index]
                        .heavy_atoms[0]
                        .residue_label
                    )
                    atom_chain_label = (
                ...
                self.bfactor_df.loc[
                    (self.bfactor_df["label"] == atom_label)
                    & (self.bfactor_df["residue_label"] == atom_residue_label)
                    & (self.bfactor_df["chain_label"] == atom_chain_label),
                    "atom_index",
                ] = atom.index

        self.protein.normalise_labels(mode="molecule")

        if self.bfactor:
            for atom in self.protein.atoms:
                self.bfactor_df.loc[
                    self.bfactor_df["atom_index"] == atom.index, "normalised_atom_label"
                ] = atom.label
        self.hbond_df = get_hbond_df(self.protein)
        self.hbond_mismatch_df = get_hbond_mismatch_df(
            self.protein, self.interaction_cutoff
        )
        ...
        self.rdkit_ligand = Chem.MolFromMol2Block(
            self.csd_ligand.to_string("mol2"), removeHs=True
        )
        ...
        self.protein_df = rdkit_match_protein_atoms(
            self.rdkit_protein,
            self.protein,
            self.protein_atom_types_df,
            pdb=False,
        )
        self.protein_df = self.protein_df[
            self.protein_df["protein_atom_label"].isin(
                self.ligand_df["ligand_atom_label"]
            )
            == False
        ]
        self.csd_ligand.remove_hydrogens()
    ...
    def contact_df(self):
        ...
        central_ligand_atoms = [
            protein_atoms[int(at.partial_charge)]
            for at in self.csd_ligand_atoms
        ...
        los_contacts_df = _contact_df_loop(
            ...
            csd_ligand_entry=self.csd_ligand_entry,
            ligand_file=self.input_ligand,
            interaction_cutoff=self.interaction_cutoff,
        )
        ...
        #                                        (los_contacts_df['is_hbond'] == False)
        hbond_mismatch_list_1 = (
            self.hbond_mismatch_df[
                [
                    "atom1_label",
                    "atom1_residue_label",
                    "atom2_label",
        ...
        los_contacts_df["is_hbond_mismatch"] = (
            ...
            .agg(",".join, axis=1)
            .isin(hbond_mismatch_list_2)
        )
        return los_contacts_df

    def delta_apolar_sasa(self):
        bound_ligand_atoms = [
            self.protein.atom(l) for l in [a.label for a in self.csd_ligand_atoms]
        ]
        delta_apolar_sasa = calculate_delta_apolar_sasa(
            bound_ligand_atoms, self.csd_ligand_atoms
        )
        return delta_apolar_sasa
...
class CsdDescriptorsFromPDB(object):
    def __init__(
        ...
        if Path(protein_file).is_file():
            self.protein = protein.Protein.from_file(str(protein_file))
            self.protein.kekulize()

        waters = list(self.protein.waters)
        if strucid:
            self.protein.identifier = strucid
        ...
        if ligand is None:
            ...
            self.rdkit_ligand = Chem.MolFromMol2Block(
                self.csd_ligand.to_string(), removeHs=True, sanitize=True
            )
        ...
        if only_binding_site:
            self.protein = _cut_out_binding_site_by_residue_df(
                self.protein, self.project_home
            )

        self.protein.add_ligand(self.csd_ligand)
        ligand_index = len(self.protein.ligands) - 1
        for w in waters:
            try:
                self.protein.add_ligand(w)
            except:
                continue

        if self.bfactor:
            pdb_protein_file = list(
                (Path(self.project_home) / Path("tmp_aligned_for_MOE_sanitized")).glob(
                    f"{molecule.identifier}*.pdb"
                )
            )[0]
            self.bfactor_df = get_b_factors(pdb_protein_file)
            for atom in self.protein.atoms:
                self.bfactor_df.loc[
                    (self.bfactor_df["label"] == atom.label)
                    & (self.bfactor_df["residue_label"] == atom.residue_label)
                    & (self.bfactor_df["chain_label"] == atom.chain_label),
                    "atom_index",
                ] = atom.index
        self.protein.normalise_labels(mode="molecule")
        ...
        self.protein = utils.assign_index_to_atom_partial_charge(self.protein)
        self.csd_ligand = self.protein.ligands[ligand_index]
        self.csd_ligand.remove_hydrogens()
        self.csd_ligand_atoms = self.csd_ligand.atoms
        ...
        # hbond mismatch: donor...donor or acceptor...acceptor contact
        los_contacts_df["is_hbond_mismatch"] = (
            los_contacts_df["ligand_atom_is_acceptor"]
        ) & (los_contacts_df["los_atom_is_acceptor"]) & (
            los_contacts_df["is_hbond"] == False
        ) | (
            los_contacts_df["ligand_atom_is_donor"]
    ...
    def delta_apolar_sasa(self):
        try:
            bound_ligand_atoms = [
                self.protein.atom(a.label) for a in self.csd_ligand_atoms
            ]
        except RuntimeError:
            ligand_res_at_labels = [
                (a.residue_label, a.label) for a in self.csd_ligand_atoms
            ]
            bound_ligand_atoms = [
                a
                for a in self.protein_atoms
                if (a.residue_label, a.label) in ligand_res_at_labels
            ]
...
class CsdDescriptorsFromCCDC(object):
    def __init__(
        self,
        ccdc_target,
        ccdc_ligand,
        ...
        for a in self.protein.atoms:
            a.label = a.atomic_symbol + str(a.index)
...
class PlotDescriptors(object):
    def __init__(self, input_file):
        ...
        for target in [
            ...
            "hivrt",
            "hivpr",
        ]:  # 'ampc', 'cp3a4', 'gcr', ','cxcr4 'kif11', 'hivrt', 'hivpr'

            target_df = pd.read_parquet(Path(target) / "GOLD" / input_file)
            if "gold_ligand_name" not in target_df.columns:
                target_df["gold_ligand_name"] = target_df.apply(
                    lambda row: row["ligand_file"].split("_")[-2], axis=1
                )
                target_df.loc[:, "target"] = target
                target_df.to_parquet(
                    Path(target) / "GOLD" / input_file, compression="gzip"
                )

            target_contact_df = pd.read_parquet(
                Path(target) / "GOLD" / "combined_contacts.gzip"
            )
            self.fitness_str = [s for s in target_contact_df.columns if "Fitness" in s][
                0
            ]
            if "gold_ligand_name" not in target_contact_df.columns:
                ...
                target_contact_df.to_parquet(
                    Path(target) / "GOLD" / "combined_contacts.gzip",
                    compression="gzip",
                )
            target_df = target_df.join(
                target_contact_df.drop_duplicates("ligand_file")[
                    ["ligand_file", self.fitness_str]
                ].set_index("ligand_file"),
                on="ligand_file",
            )
            target_df = target_df[
                (target_df["competitive_contacts_per_atom"] > 0)
                | (target_df["non_competitive_contacts_per_atom"] > 0)
            ].sort_values(by=self.fitness_str, ascending=False)
            for delta in ["", "_d01", "_d03", "_d05"]:
                target_df[f"% non-competitive{delta}"] = target_df[
                    f"non_competitive_contacts_per_atom{delta}"
                ] / (
                    target_df[f"competitive_contacts_per_atom"]
                    + target_df[f"non_competitive_contacts_per_atom"]
                )
        ...
        self.all_targets_top500_df = pd.concat(
            self.all_targets_top500_df, ignore_index=True
        ).astype({self.fitness_str: float})
        self.all_targets_df = pd.concat(self.all_targets_df, ignore_index=True).astype(
            {self.fitness_str: float}
        )
        self.all_targets_contacts_df = pd.concat(
            self.all_targets_contacts_df, ignore_index=True
        ).astype({self.fitness_str: float})
        columns = [
            "identifier",
            "resolution",
            "non_competitive_contact_count",
            "competitive_contact_count",
            "non_competitive_contacts_per_atom",
            "competitive_contacts_per_atom",
            "rf_max",
            "rf_min",
        ...
        self.in_house_df = pd.read_parquet(
            Path(self.rf_home) / "full_p2cq_roche_oct2019_rf_extended.gzip",
            columns=columns,
        ).drop_duplicates("identifier")
        self.in_house_df = self.in_house_df[self.in_house_df["resolution"] <= 2.5]
        self.in_house_df = self.in_house_df[
            (self.in_house_df["non_competitive_contacts_per_atom"] > 0)
        ...
        self.public_df = pd.read_parquet(
            Path(self.rf_home) / "full_p2cq_pub_oct2019_rf_extended.gzip",
            columns=columns
            + [
                "ligand_rscc",
                "ligand_altcode",
        ...
        self.public_df = self.public_df[
            (self.public_df["resolution"] <= 2.5)
            & (self.public_df["ligand_rscc"] >= 0.8)
            & (self.public_df["ligand_altcode"] == " ")
            & (self.public_df["ligand_avgoccu"] == 1)
        ]
        self.public_df = self.public_df[
            (self.public_df["non_competitive_contacts_per_atom"] > 0)
    ...
    def data_distribution(self):
        ...
        for target in self.all_targets_df["target"].unique():
            plot_df = pd.concat(
                [
                    self.all_targets_df[self.all_targets_df["target"] == target],
                    self.public_df,
                    self.in_house_df,
                ]
            )
            ...
            boxplot = sns.boxplot(
                x="category", y="value", hue="variable", data=long_df, showfliers=False
            )
        ...
        plot_df = pd.concat(
            [
                self.public_df.loc[:, ["target", "rf_total", "category"]],
        ...
        violinplot = sns.violinplot(
            x="target", y="rf_total", hue="category", data=plot_df, showfliers=False
        )
        ...
        long_df = pd.melt(
            plot_df,
            id_vars=["target", "category"],
            value_vars=[
                "non_competitive_contacts_per_atom",
                "competitive_contacts_per_atom",
            ],
        )
        long_df["interaction_category"] = long_df.apply(
            lambda row: row["variable"] + "_" + row["category"], axis=1
        )
        boxplot = sns.boxplot(
            x="target",
            y="value",
        ...
        boxplot = sns.boxplot(
            x="target", y="rf_max", hue="category", data=plot_df, showfliers=False
        )
        boxplot.figure.savefig("boxplot_top500_rf_max.png", dpi=600)
        plt.clf()

        boxplot = sns.boxplot(
            x="target", y="rf_min", hue="category", data=plot_df, showfliers=False
        )
    ...
    def _rescored_fitness(self, target_contact_df, weight=10):
        ...
        rf_score_list = []
        ligand_file_list = []
        target_contact_df[f"{weight}xlg_rf_total"] = weight * np.log(
            target_contact_df["rf_total"]
        )
        grouped = target_contact_df.groupby(["ligand_file", self.fitness_str])[
            f"{weight}xlg_rf_total"
        ].sum()
        ...
        rescored_fitness_df = pd.DataFrame(
            {
                "ligand_file": ligand_file_list,
                "rescored_fitness": rescored_fitness_list,
    ...
    def _enrichment_factor(self, target_df, score_label):
        ...
        hits_total_dataset = (
            target_df.drop_duplicates(subset="ligand_file")["category"]
            .str.count("actives")
            .sum()
        )
        denominator = hits_total_dataset / total_dataset_size
        target_df = target_df.sort_values(
            by=score_label, ascending=False
        ).drop_duplicates(subset="gold_ligand_name")
    ...
    def enrichment_barplot(self, weight):
        ...
        ef_1pc_df = []
        all_targets_contacts_df = self.all_targets_contacts_df[
            [
                "ligand_file",
                "category",
                "target",
        ...
        meta = {
            ...
            "category": np.str_,
            "target": np.str_,
            self.fitness_str: np.float64,
            "gold_ligand_name": np.str_,
            "rf_total": np.float64,
            "is_primary": np.bool_,
        }
        all_targets_contacts_df = all_targets_contacts_df.astype(meta)
        for target in all_targets_contacts_df["target"].unique():
            target_contact_df = all_targets_contacts_df[
                all_targets_contacts_df["target"] == target
            ].copy()
            target_df = self.all_targets_df[
                self.all_targets_df["target"] == target
            ].copy()

            rescored_fitness = self._rescored_fitness(target_contact_df, weight)
            target_df = target_df.join(
                rescored_fitness.set_index("ligand_file"), on="ligand_file"
            )
            ...
            ef_1pc_RF_torsion_rescore = self._enrichment_factor(
                target_df, "ChemPLP_RF_torsion_rescore"
            )
            ...
            ef_1pc_df.append(
                pd.DataFrame(
                    {
                        "EF_1%": [ef_1pc, ef_1pc_rescored, ef_1pc_RF_torsion_rescore],
                        "target": [target, target, target],
                        "Scoring_method": [
                            "ChemPLPScore",
                            "ChemPLPScore_RF_rescore",
        ...
        print("plotting...")
        ef_plot = sns.barplot(
            x="target", y="EF_1%", hue="Scoring_method", data=ef_1pc_df
        )
        ef_plot.set(ylabel="EF 1%")
        # ef_plot.legend(loc='center', bbox_to_anchor=(1.2, 0.5), prop={'size': 8})
        ef_plot.figure.savefig(
            f"ef_1pc_barplot_weight_{weight}.png", dpi=600, bbox_inches="tight"
        )
        plt.clf()

    def enrichment_plot(
        self,
        unfav_parameter=None,
        fav_parameter=None,
        fav_parameter_max=1,
        unfav_parameter_max=1,
    ):
        ...
        for target in self.all_targets_df["target"].unique():
            ...
            total_dataset_size = target_df.drop_duplicates(subset="ligand_file").shape[
                0
            ]
            hits_total_dataset = (
                target_df.drop_duplicates(subset="ligand_file")["category"]
            ...
            for index, threshold in enumerate(thresholds):
                ...
                if unfav_parameter:
                    filtered_df.loc[
                        filtered_df[unfav_parameter] > threshold, "fitness"
                    ] = 0
                    # filtered_df.loc[filtered_df['Red'] > 1, 'fitness'] = 0
                if fav_parameter:
                    filtered_df.loc[
                        filtered_df[fav_parameter]
                        < thresholds[len(thresholds) - index - 1],
                        "fitness",
                    ] = 0

                target_df_filtered = filtered_df.sort_values(
                    by="fitness", ascending=False
                ).drop_duplicates(subset="gold_ligand_name")
                sample_number = target_df_filtered[
                    target_df_filtered["fitness"] > 0
                ].shape[0]
                ...
                if sample_number > 0.01 * target_df_filtered.shape[0]:
                    ...
                    ef_1pc_df.loc[
                        ef_1pc_df["threshold"] == threshold, "total_count_top_1pc"
                    ] = len(top_1pc)
                else:
        ...
        ef_plot = sns.scatterplot(
            x="threshold", y="value", hue="variable", data=plot_df
        )
        ef_plot.set(ylabel="EF 1%")
        ef_plot.legend(loc="center", bbox_to_anchor=(1.2, 0.5), prop={"size": 8})
        ef_plot.figure.savefig(
            f"ef_1pc_{unfav_parameter}_{fav_parameter}.png",
            dpi=600,
            bbox_inches="tight",
        )
        plt.clf()
...
def main():
    # csd_describer = CsdDescriptorsFromGold(
    #     "./0ATX2_010/gold_soln_ligand_m1_1.sdf", "./0ATX2_010/api_gold.conf"
    # )
    csd_describer = CsdDescriptorsFromPDB(
        "0_output.pdb",
        "magl_60cmpds_quinazolinones_prep_1_1.sdf",
    )
