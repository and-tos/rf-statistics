...
class TotalRf(object):
    def __init__(
        ...
        protein,
        protein_atom_types,
        global_ligand_lookup_alpha,
        global_ligand_lookup_h,
        global_ligand_lookup_distance,
        global_protein_lookup_alpha,
        global_protein_lookup_h,
        global_protein_lookup_distance,
        expected=10,
    ):
        """
        ...
        self.global_ligand_lookup_distance = global_ligand_lookup_distance
        self.global_protein_lookup_alpha = global_protein_lookup_alpha
        self.global_protein_lookup_h = global_protein_lookup_h
        self.global_protein_lookup_distance = global_protein_lookup_distance
    ...
    def rf_alpha(self, lookup, alphas, ligand_atom_type, pat):
        ...
        try:
            ...
            for alpha in alphas:
                _filtered_alpha_df = alpha_df[
                    (alpha_df["alpha_max"] > alpha) & (alpha_df["alpha_min"] <= alpha)
                ].squeeze()
    ...
    def rf_distance(self, lookup, h, ligand_atom_type, pat):
        """
        ...
        try:
            ...
            h_df = h_df[
                (h_df["distance_max"] > h) & (h_df["distance_min"] <= h)
            ].squeeze()
            if h_df.shape[0] != 0:
                rf_h = [h_df["rf"]]
            ...
            if h_df.shape[0] == 0 or len(rf_h) == 0 or pat == "other_ligand":
                rf_h = [1]
                rf_h_high = [0]
                rf_h_low = [0]
    ...
    def rf_ligand(self, ligand_alphas, distance, ligand_atom_type, pat, ligand_h=None):
        """
        ...
        if len(ligand_alphas) == 0:
            ...
            (
                rf_ligand_alpha,
                rf_ligand_alpha_error,
                alpha_interaction_types,
            ) = self.rf_alpha(
                self.global_ligand_lookup_alpha, ligand_alphas, ligand_atom_type, pat
            )
        rf_ligand.append(rf_ligand_alpha)
        rf_ligand_errors.append(rf_ligand_alpha_error)

        # RF distance
        rf_ligand_distance, rf_ligand_distance_error = self.rf_distance(
            self.global_ligand_lookup_distance, distance, ligand_atom_type, pat
        )

        rf_ligand.append(rf_ligand_distance)
        rf_ligand_errors.append(rf_ligand_distance_error)

        if ligand_h is not None and ligand_h == ligand_h:
            rf_ligand_h, rf_ligand_h_error, h_interaction_types = self.rf_h(
                self.global_ligand_lookup_h, ligand_h, ligand_atom_type, pat
            )
        ...
        rf_ligand_dict = {
            "rf_ligand": rf_ligand,
            "rf_ligand_error": rf_ligand_error,
            "rf_ligand_alpha": rf_ligand_alpha,
            "rf_ligand_alpha_error": rf_ligand_alpha_error,
            "rf_ligand_distance": rf_ligand_distance,
            "rf_ligand_distance_error": rf_ligand_distance_error,
            "rf_ligand_h": rf_ligand_h,
            "rf_ligand_h_error": rf_ligand_h_error,
            "ligand_interactions": ligand_interactions,
        }
        return rf_ligand_dict

    def rf_protein(
        self, protein_alphas, distance, ligand_atom_type, pat, protein_h=None
    ):
        """
        ...
        if len(protein_alphas) == 0:
            ...
            (
                rf_protein_alpha,
                rf_protein_alpha_error,
                alpha_interaction_types,
            ) = self.rf_alpha(
                self.global_protein_lookup_alpha, protein_alphas, ligand_atom_type, pat
            )
        rf_protein.append(rf_protein_alpha)
        rf_protein_errors.append(rf_protein_alpha_error)

        rf_protein_distance, rf_protein_distance_error = self.rf_distance(
            self.global_protein_lookup_distance, distance, ligand_atom_type, pat
        )

        if protein_h is not None and protein_h == protein_h:
            rf_protein_h, rf_protein_h_error, h_interaction_types = self.rf_h(
                self.global_protein_lookup_h, protein_h, ligand_atom_type, pat
            )
        ...
        rf_protein_dict = {
            "rf_protein": rf_protein,
            "rf_protein_error": rf_protein_error,
            "rf_protein_alpha": rf_protein_alpha,
            "rf_protein_alpha_error": rf_protein_alpha_error,
            "rf_protein_distance": rf_protein_distance,
            "rf_protein_distance_error": rf_protein_distance_error,
            "rf_protein_h": rf_protein_h,
            "rf_protein_h_error": rf_protein_h_error,
    ...
    def return_rf(
        ...
        if not hasattr(los_atom, "los_at"):
            ...
            from rf_statistics.utils import return_atom_type

            pat = return_atom_type(
                los_atom,
                hit_protein=self.protein,
                protein_atom_types_df=self.protein_atom_types,
            )
        else:
            pat = los_atom.los_at

        rf_protein_dict = self.rf_protein(
            protein_alphas, distance, ligand_atom_type, pat, protein_h
        )
        rf_ligand_dict = self.rf_ligand(
            ligand_alphas, distance, ligand_atom_type, pat, ligand_h
        )

        rf_total_error = error_propagation(
            [rf_protein_dict["rf_protein"], rf_ligand_dict["rf_ligand"]],
