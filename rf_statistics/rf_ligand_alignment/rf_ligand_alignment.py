...
import os
import pandas as pd
from rf_statistics import utils
...
def parse_args():
    ...
    parser.add_argument(
        "--protein_file", help="Protein file.", type=str, default="mol.sdf"
    )

    parser.add_argument(
        "--ligand_file", help="Ligand file.", type=str, default="ligand.sdf"
    )
...
def _make_new_molecule(atoms):
    ...
    keep_bonds = [
        bond for bond in flat_bonds if bond.atoms[0] in atoms and bond.atoms[1] in atoms
    ]
...
class RfLookup(object):
    def __init__(self, dbs="internal", protein_atom_type="N_pi_don"):
        ...
        for db in dbs.split(","):
            ...
            self.lookup_paths.append(PurePath(self.rf_home, rf_db))
    ...
    def return_competitive_df_from_geometry(self):
        ...
        for lookup_path in self.lookup_paths:
            competitive_df = pd.read_parquet(
                lookup_path,
                columns=[
                    "protein_atom_type",
                    "rf_total_error",
            ...
            competitive_df = competitive_df.loc[
                (competitive_df.loc[:, "protein_atom_type"] == self.protein_atom_type)
                & (
                    competitive_df.loc[:, "rf_total"]
                    - competitive_df.loc[:, "rf_total_error"]
                    > 1
                )
            ].copy()
            competitive_df = competitive_df[
                (competitive_df["is_cofactor"] == False)
        ...
        project_df = competitive_df[
            competitive_df["project"].isna() == False
        ].drop_duplicates(["ligand_smiles", "project"])
        no_project_df = competitive_df[competitive_df["project"].isna()]
        competitive_df = pd.concat(
            [project_df, no_project_df], ignore_index=True
        ).sort_values(by="rf_total", ascending=False)
        return competitive_df

    def _trim_reference_protein(
        self, reference_protein, reference_protein_index, env=3
    ):
        ref_atoms_to_keep = set([reference_protein.atom("_RP_XX")])
        for env in range(env):
            ref_atoms_to_keep.update(
                [
                    n
                    for sublist in [a.neighbours for a in ref_atoms_to_keep]
                    for n in sublist
                ]
            )
    ...
    def _prepare_reference_protein(
        self,
        reference_protein,
        reference_protein_index,
        reference_protein_dummy_coordinates,
    ):
        if ".mol2" in reference_protein or ".sdf" in reference_protein:
            reference_protein = protein.Protein.from_entry(
                io.EntryReader(reference_protein)[0]
            )

        elif len(reference_protein) == 8:
            rdr = io.EntryReader(
                str(Path(self.rf_home) / "full_p2cq_pub_oct2019.csdsql")
            )
            reference_protein = protein.Protein.from_entry(rdr.entry(reference_protein))

        elif len(reference_protein) == 9:
            rdr = io.EntryReader(
                str(Path(self.rf_home) / "full_p2cq_roche_oct2019.csdsql")
            )
            reference_protein = protein.Protein.from_entry(rdr.entry(reference_protein))
        ...
        if len(reference_protein.atom("_RP_XX").neighbours) == 1:
            ...
            protein_cutoff = 1
        reference_protein = self._trim_reference_protein(
            reference_protein, reference_protein_index, protein_cutoff
        )
        dummy_atom = molecule.Atom(
            coordinates=reference_protein_dummy_coordinates, label="XY"
        )
    ...
    def _get_matrices_and_for(self, hit_protein_ex_ligand):
        ...
        point_group_analyzer = descriptors.MolecularDescriptors.point_group_analysis(
            transformed_mol, return_matrices=True
        )
    ...
    def _get_overlay_atoms(self, reference_protein, hit_protein_ex_ligand):
        ...
        hit_protein_ex_ligand_neighbours = hit_protein_ex_ligand.atom(
            "_P_XX"
        ).neighbours
        if len(reference_protein_neighbours) >= len(hit_protein_ex_ligand_neighbours):
            overlay_atoms = [
                (reference_protein.atom("_RP_XX"), hit_protein_ex_ligand.atom("_P_XX"))
            ] + [
        ...
        if (
            len(reference_protein_neighbours)
            == len(hit_protein_ex_ligand_neighbours)
            == 1
        ):
            if len(reference_protein_neighbours[0].neighbours) >= len(
                hit_protein_ex_ligand_neighbours[0].neighbours
            ):
                overlay_atoms = overlay_atoms + [
                    (reference_protein_neighbours[0].neighbours[i], n)
                    for i, n in enumerate(
                        hit_protein_ex_ligand_neighbours[0].neighbours
                    )
                    if "_P_XX" not in n.label
                ]
            else:
                overlay_atoms = overlay_atoms + [
                    (n, hit_protein_ex_ligand_neighbours[0].neighbours[i])
                    for i, n in enumerate(reference_protein_neighbours[0].neighbours)
                    if "_RP_XX" not in n.label
                ]
    ...
    def _align_ligand(
        self, iterrow, reference_protein, reference_ligand, ligand_atom_index, fragments
    ):
        index, row = iterrow
        rdr = io.EntryReader(
            [os.path.join(self.rf_home, db) for db in self.structure_db]
        )
        ...
        hit_protein = utils.assign_index_to_atom_label(hit_protein)
        ...
        if len(hit_protein.atom("_P_XX").neighbours) == 1:
            ...
            "pi" in self.protein_atom_type
            and len(hit_protein.atom("_P_XX").neighbours) < 3
        ):
            protein_cutoff = 2
        else:
            protein_cutoff = 1
        for env in range(protein_cutoff):
            protein_atoms_to_keep.update(
                [
                    n
                    for sublist in [a.neighbours for a in protein_atoms_to_keep]
                    for n in sublist
                ]
            )
        ...
        hit_protein_ex_ligand = [
            c for c in hit_protein.components if "_Z" not in c.atoms[0].label
        ][0]

        overlay_atoms = self._get_overlay_atoms(
            reference_protein, hit_protein_ex_ligand
        )

        overlaid_hit_protein = descriptors.MolecularDescriptors.overlay(
            reference_protein,
            hit_protein,
            overlay_atoms,
            with_symmetry=False,
        ...
        overlaid_hit_protein_ex_ligand = [
            c for c in overlaid_hit_protein.components if "_Z" not in c.atoms[0].label
        ][0]

        rotation_matrices, frame_of_ref = self._get_matrices_and_for(
            overlaid_hit_protein_ex_ligand
        )
        distance = descriptors.MolecularDescriptors.atom_distance(
            reference_protein.atom("XY"), overlaid_hit_protein.atom("_Z_XX")
        )
        aligned_ligands_df = pd.DataFrame(
            {"aligned_molecule": [overlaid_hit_protein], "distance": [distance]}
        )
        for rotation_matrix in rotation_matrices:
            t_hit_protein = overlaid_hit_protein.copy()
            move_to_for = t_hit_protein.Transformation.from_rotation_and_translation(
                ((1, 0, 0), (0, 1, 0), (0, 0, 1)), frame_of_ref[0]
            ).inverse()
            move_to_for2 = t_hit_protein.Transformation.from_rotation_and_translation(
                frame_of_ref[1], (0, 0, 0)
            )
            sym_op = t_hit_protein.Transformation.from_rotation_and_translation(
            ...
            protein_atom_distance = descriptors.MolecularDescriptors.atom_distance(
                reference_protein.atom("_RP_XX"), t_hit_protein.atom("_P_XX")
            )
            if protein_atom_distance > 0.5:
                continue

            t_hit_protein_ex_ligand = [
                c for c in t_hit_protein.components if "_Z" not in c.atoms[0].label
            ][0]
            rmsd_atoms = self._get_overlay_atoms(
                reference_protein, t_hit_protein_ex_ligand
            )
            protein_rmsd = descriptors.MolecularDescriptors.rmsd(
                reference_protein,
                t_hit_protein_ex_ligand,
                atoms=rmsd_atoms,
                with_symmetry=False,
            ...
            ligand_atom_distance = descriptors.MolecularDescriptors.atom_distance(
                reference_protein.atom("XY"), t_hit_protein.atom("_Z_XX")
            )
            if ligand_atom_distance > 2:
                continue

            aligned_ligands_df = aligned_ligands_df.append(
                {
                    "aligned_molecule": t_hit_protein,
        ...
        hit_protein = aligned_ligands_df.loc[
            aligned_ligands_df["ligand_atom_distance"].idxmin(), "aligned_molecule"
        ]
        ...
        for env in range(3):
            atoms_to_keep.update(
                [
                    n
                    for sublist in [a.neighbours for a in atoms_to_keep]
                    for n in sublist
                ]
            )
        ...
        if len(reference_ligand_atom.neighbours) <= len(
            fragment_central_atom.neighbours
        ):
        ...
        ligand_rmsd = descriptors.MolecularDescriptors.rmsd(
            reference_ligand, fragment, atoms=overlay_atoms, with_symmetry=False
        )

        pl_complex.attributes = {
            "rf_total": row["rf_total"],
            "identifier": identifier,
            "fragment_rmsd": ligand_rmsd,
        }
    ...
    def align_ligands(
        ...
        if nproc > 1:
            parallel_return_structure_df = partial(
                self._align_ligand,
            ...
            reference_protein = self._prepare_reference_protein(
                reference_protein,
                reference_protein_index,
                reference_protein_dummy_coordinates,
            )
            ...
            for iterrow in competitive_df.iterrows():
                try:
                    pl_complex, ligand, fragment = self._align_ligand(
                        iterrow,
                        reference_protein,
                        reference_ligand,
                        ligand_atom_index,
                ...
                if len(fragments) >= search_limit:
                    break
            with io.EntryWriter("ligands.sdf") as wl, io.EntryWriter(
                "fragments.sdf"
            ) as wf, io.EntryWriter("complex.sdf") as wc:
                for f in fragments:
                    if f:
                        wf.write(f)
                for lig in ligands:
...
def main():
    ...
    if type(args.alignment_coordinates) == str:
        alignment_coordinates = tuple(
            [float(coord) for coord in args.alignment_coordinates.split(",")]
        )
    else:
    ...
    looker.align_ligands(
        competitive_df,
        args.protein_file,
        args.ligand_file,
        args.protein_atom_index,
        args.ligand_atom_index,
        alignment_coordinates,
