...
with io.EntryReader(public_db) as public_rdr, io.EntryReader(
    roche_db
) as roche_rdr, io.EntryWriter("central_ligands.sdf") as w:
        ...
        try:
            if len(strucid) == 4:
                binding_site_identifiers = [
                    i for i in public_identifiers if strucid.upper() in i
                ]
                entries = [
                    public_rdr.entry(binding_site_identifier)
                    for binding_site_identifier in binding_site_identifiers
                ]
            else:
                binding_site_identifiers = [
                    i for i in roche_identifiers if strucid.upper() in i
                ]
                entries = [
                    roche_rdr.entry(binding_site_identifier)
                    for binding_site_identifier in binding_site_identifiers
                ]
            for e in entries:
                p = protein.Protein.from_entry(e)
                central_ligand = [c for c in p.components if "_Z" in c.atoms[0].label][
                    0
                ]
