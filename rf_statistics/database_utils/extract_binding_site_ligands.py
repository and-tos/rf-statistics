...
def parse_args():
    ...
    parser.add_argument(
        "-o", "--output", help="Output filename.", default="ligands.sdf"
    )
...
def main():
    ...
    if args.input == "internal":
        db = "full_p2cq_roche_jul2023.csdsql"
    else:
        db = "full_p2cq_pub_jul2023.csdsql"
    rdr = io.MoleculeReader(db)

    cofactor_list = protein.Protein.known_cofactor_codes() + [
        "AMP",
        "ADP",
        "ATP",
        "GMP",
    ...
    with io.EntryWriter(args.output) as w:
        for ccdc_mol in rdr:
            central_ligand = [
                c for c in ccdc_mol.components if "_Z" in c.atoms[0].label
            ]
            if central_ligand:
                central_ligand = central_ligand[0]
                if len(central_ligand.heavy_atoms) >= 10:
                    if (
                        central_ligand.heavy_atoms[0].residue_label[:3]
                        not in cofactor_list
                    ):
                        central_ligand.identifier = ccdc_mol.identifier
                        central_ligand_entry = entry.Entry.from_molecule(central_ligand)
                        central_ligand_entry.attributes["strucid"] = (
                            ccdc_mol.identifier.split("_")[0]
                        )
