...
rdr = io.EntryReader(os.path.join(rf_home, db))

df = df[df["query_match"] == 1]

alpha_mins = range(0, 120, 20)
alpha_mins_df = pd.DataFrame(
    columns=["alpha_min", "alpha_max", "tyr_count", "ser/thr_count"]
)
...
for index, row in df.iterrows():
    ...
    alpha_ligand = 180 - descriptors.MolecularDescriptors.atom_angle(
        chlorine_neighbour_atom, chlorine_atom, carbon_atom
    )

    for alpha_min in reversed(alpha_mins):
        if alpha_protein > alpha_min:
            alpha_mins_df.loc[alpha_mins_df["alpha_min"] == alpha_min, column_name] = (
                alpha_mins_df.loc[alpha_mins_df["alpha_min"] == alpha_min, column_name]
                + 1
            )
            break

    contact_angle_df = contact_angle_df.append(
        {
            "alpha_protein": alpha_protein,
            "alpha_ligand": alpha_ligand,
            "residue": O_mix_residue[:3],
            "identifier": identifier,
        },
        ignore_index=True,
    )

contact_angle_plot = sns.scatterplot(
    data=contact_angle_df,
    x="alpha_protein",
    y="alpha_ligand",
    hue="residue",
    palette=sns.color_palette("colorblind", len(contact_angle_df["residue"].unique())),
)
contact_angle_plot.set_xlabel(r"$\alpha$ C-O_mix...Cl")
contact_angle_plot.set_ylabel(r"$\alpha$ C-Cl...$C_{O_mix}$")
contact_angle_plot.figure.savefig(
    "contact_angle_plot.png", dpi=300, bbox_inches="tight"
)
plt.close()
