...
def parse_args():
    ...
    parser.add_argument(
        "-nproc", help="Number of parallel processes for multiprocessing.", default=24
    )

    parser.add_argument("-db", help="DB internal or external", default="internal")
...
def calculate_buried_surface(mol):
    ...
    try:
        ...
        bound_ligand_sasa = np.sum(
            [a.solvent_accessible_surface() for a in ligand_atoms]
        )
        ...
        unbound_ligand_sasa = np.sum(
            [a.solvent_accessible_surface() for a in mol.atoms]
        )
...
def single_processing():
    ...
    df = pd.DataFrame(
        {
            "bound_ligand_sasa": bound_ligand_sasa_list,
            "unbound_ligand_sasa_list": unbound_ligand_sasa_list,
            "pc_buried_list": pc_buried_list,
            "identifiers": identifiers,
            "pc_implicit_sasa": pc_implicit_sasa_list,
            "implicit_sasa": implicit_sasa_list,
        }
    )
...
def multi_processing(db, nproc=24):
    ...
    output = [
        ...
        for i in pool.map(
            parallel_buriedness,
            range(len(io.EntryReader("full_p2cq_pub_oct2019.csdsql"))),
        )
        if i is not None
    ]
