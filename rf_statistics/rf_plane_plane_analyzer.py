...
import __future__
from rf_statistics.rf_postprocessing import distance_dependency
from rf_statistics import total_rf_calculator
import os
...
class RfPlanePlane(object):
    def __init__(
        ...
        self.rf_home = Path(rf_home).absolute()
        self.cwd = os.getcwd()
        self.protein_plane = protein_plane
        self.ligand_plane = ligand_plane

    def _calculate_ligand_plane(self):
        os.chdir(Path("rf_statistics") / self.ligand_atom_type / "query_atom")
    ...
    def _calculate_protein_plane(self):
        os.chdir(Path("rf_statistics") / self.ligand_atom_type / self.protein_atom_type)
        # if not os.path.isfile('statistics_h_plane_dependency.csv'):
        distance_dependency(
            "h",
            "protein",
    ...
    def _calculate_rf_total(self, rf_dict):
        ...
        rf_ligand_uncertainty = total_rf_calculator.error_propagation(
            [rf_ligand_alpha, rf_ligand_h],
            [rf_ligand_alpha_uncertainty, rf_ligand_h_uncertainty],
        )
        rf_protein = gmean([rf_protein_alpha, rf_protein_h])
        rf_protein_uncertainty = total_rf_calculator.error_propagation(
            [rf_protein_alpha, rf_protein_h],
            [rf_protein_alpha_uncertainty, rf_protein_h_uncertainty],
        )
        rf_total = gmean([rf_ligand, rf_protein])
        rf_total_uncertainty = total_rf_calculator.error_propagation(
            [rf_ligand, rf_protein], [rf_ligand_uncertainty, rf_protein_uncertainty]
        )
    ...
    def _get_rf_df(self):
        if self.protein_plane:
            ...
            protein_df["rf_uncertainty"] = (
                protein_df["rf_high"] - protein_df["rf_low"]
            ) / 2

        else:
            protein_plane_limits = [None, None]

        protein_unconstrained_df = pd.read_csv(
            Path("rf_statistics")
            / self.ligand_atom_type
            / self.protein_atom_type
            / "rf.csv"
        ...
        protein_unconstrained_df["rf_uncertainty"] = (
            protein_unconstrained_df["rf_high"] - protein_unconstrained_df["rf_low"]
        ) / 2
        ...
        if self.ligand_plane:
            ...
            ligand_df["rf_uncertainty"] = (
                ligand_df["rf_high"] - ligand_df["rf_low"]
            ) / 2

        else:
            ligand_plane_limits = [None, None]

        ligand_unconstrained_df = pd.read_csv(
            Path("rf_statistics") / self.ligand_atom_type / "query_atom" / "rf.csv"
        )

        ligand_unconstrained_df["rf_uncertainty"] = (
            ligand_unconstrained_df["rf_high"] - ligand_unconstrained_df["rf_low"]
        ) / 2
        ...
        for i in plane_limits:
            ...
            rf_ligand_alpha = ligand_unconstrained_df[
                ligand_unconstrained_df["atom_type"] == self.protein_atom_type
            ]["rf"].to_numpy()[0]
            ...
            rf_protein_alpha = protein_unconstrained_df[
                protein_unconstrained_df["atom_type"] == "query_match"
            ]["rf"].to_numpy()[0]
            ...
            rf_dict = {
                ...
                "rf_ligand_alpha_uncertainty": rf_ligand_alpha_uncertainty,
                "rf_ligand_h": rf_ligand_h,
                "rf_ligand_h_uncertainty": rf_ligand_h_uncertainty,
                "rf_protein_alpha": rf_protein_alpha,
                "rf_protein_alpha_uncertainty": rf_protein_alpha_uncertainty,
                "rf_protein_h": rf_protein_h,
                "rf_protein_h_uncertainty": rf_protein_h_uncertainty,
            }
            ...
            rf_df = pd.DataFrame(
                {
                    "protein_h_min": [protein_h_min],
                    "ligand_h_min": [ligand_h_min],
                    "rf_total": [rf_total],
                    "rf_total_uncertainty": [rf_total_uncertainty],
...
def main():
    ...
    for ligand_atom_type in [
        "carbon_aromatic_pyrazine",
        "carbon_aromatic_phenyl",
        "nitrogen_pyrimidine",
        "nitrogen_aromatic_pyridine",
        "carbon_aromatic_pyridine_2",
        "carbon_aromatic_pyridine_3",
        "carbon_aromatic_pyridine_4",
        "carbon_aromatic_pyrimidine_2",
        "carbon_aromatic_pyrimidine_4",
        "carbon_aromatic_pyrimidine_5",
    ]:
        for protein_atom_type in ["O_pi_acc", "C_pi_carbonyl", "N_pi_don"]:
            plane_dependency_df = plane_dependency_df.append(
                RfPlanePlane(
                    protein_atom_type, ligand_atom_type, rf_home="."
                ).get_plane_dependency_df()
            )
    plane_dependency_df.to_csv(
        "heteroaromatics_amide_plane_geometry_dependency.csv", index=False
    )
