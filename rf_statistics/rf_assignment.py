...
from rf_statistics import los_protein_descriptors
from rf_statistics.los_descriptors import (
    CsdDescriptorsFromProasis,
    CsdDescriptorsFromPDB,
    CsdDescriptorsFromMol2,
)
from rf_statistics import los_descriptors
from rf_statistics.database_utils.export_contacts_surfaces import export_entry
...
def parse_args():
    ...
    parser.add_argument("-p", "--protein", help="Protein file", type=str, default="")

    parser.add_argument("-l", "--ligands", help="ligands file", type=str, default="")

    parser.add_argument(
        "--gold", help="Pass path to GOLD config file", type=str, default=""
    )

    parser.add_argument(
        "--rf_home",
        help="rf_home folder with lookup files, databases, protein_atom_types.csv and ligand_atom_types.csv",
        type=str,
        default="/LoS/protonate3d/",
    )

    parser.add_argument(
        "-v", "--verbose", help="Write out RF assignment data.", action="store_true"
    )
...
class RfAssigner(object):
    def __init__(
        ...
        if type(input) == str and len(input.split(".")) == 1:
            ...
            if len(strucid) == 5:
                dbname = "full_p2cq_roche_jul2023"
            if len(strucid) == 4:
                dbname = "full_p2cq_pub_jul2023"
            dbformat = "csdsql"

        if gold:
            self.describer = los_descriptors.CsdDescriptorsFromGold(
                input,
                gold_conf=gold,
            ...
            # self.rf_assignments = los_descriptors.return_primary_contacts_df(
            #     self.rf_assignments, self.describer.protein
            # )
            self.rotatable_bonds_num = len(
                [b for b in self.describer.csd_ligand.bonds if b.is_rotatable]
            )
            self.frozen_bonds_num = self.return_frozen_bonds_num(
                self.describer.csd_ligand
            )
            if intramolecular_protein_contacts:
                self.rf_assignments = pd.concat(
                    [
                        self.rf_assignments,
                        los_protein_descriptors.ProteinDescriptors(
                            csd_protein=self.describer.protein,
        ...
        if target_file:
            ...
            self.describer = CsdDescriptorsFromPDB(
                self.structure,
                only_binding_site=only_binding_site,
            ...
            csd_db = str(Path(self.rf_home) / Path(f"{dbname}.{dbformat}"))
    ...
    def return_frozen_bonds_num(self, ligand):
        ...
        polar_favorable_interaction_atoms = self.rf_assignments[
            (self.rf_assignments["rf_total"] >= 0.8)
            & (
                self.rf_assignments["interaction_type"].isin(
                    ["hbond_weak", "hbond_classic"]
                )
            )
        ]["ligand_atom_label"].to_list()
        polar_hbond_atoms = (
            self.describer.hbond_df["atom1_label"].to_list()
            + self.describer.hbond_df["atom2_label"].to_list()
        )

        polar_favorable_interaction_atoms = list(
            set(polar_favorable_interaction_atoms + polar_hbond_atoms)
        )
        frozen_bonds = [
            b
            for b in rotatable_bonds
            if b.atoms[0].label in polar_favorable_interaction_atoms
            or b.atoms[1].label in polar_favorable_interaction_atoms
        ]
    ...
    def visualize_in_pymol(self):
        ...
        if self.gold or self.pdb:
            for index, row in self.rf_assignments.iterrows():
                ...
                cmd.alter(
                    f"protein_atom_{protein_atom_index}",
                    f'p.atom_type = "{protein_atom_type}"',
                )
                ...
                cmd.alter(
                    f"ligand_atom_{ligand_atom_index}",
                    f'p.atom_type = "{ligand_atom_type}"',
                )
        ...
        for index, row in self.rf_assignments.iterrows():
            ...
            rf_color = return_blue_red_color(
                rf
            )  # list(self.return_blue_red_color(rf)[0:3])
            ...
            if interaction_type is not None and not row["is_intramolecular"]:
                ...
                if row["is_primary"]:
                    ...
                    intramolecular_group = (
                        f"{interaction_type}_secondary_intramolecular"
                    )
                    cmd.group(intramolecular_group, f"{index}_rf_{rf:.1f}")
        ...
        cmd.group(
            "primary_competitive",
            "hbond_classic_primary or hbond_weak_primary or multipolar_primary or "
            "hydrophobic_primary or halogen_primary or pi_primary or "
            "uncat_competitive_primary or ionic_primary",
        )
        cmd.group(
            "secondary_competitive",
            "hbond_classic_secondary or hbond_weak_secondary or multipolar_secondary or "
        ...
        cmd.group(
            "primary_non-competitive",
            "hbond_mismatch_primary or electrostatic_repulsion_primary or "
            "multipolar_mismatch_primary or desolvation_primary or "
            "pi_mismatch_primary or uncat_non_competitive_primary",
        )
        cmd.group(
            "secondary_non-competitive",
            "hbond_mismatch_secondary or electrostatic_repulsion_secondary or "
            "multipolar_mismatch_secondary or desolvation_secondary or "
            "pi_mismatch_secondary or uncat_non_competitive_secondary",
        )
        cmd.group(
            "non-competitive", "primary_non-competitive or secondary_non-competitive"
        )
        cmd.delete("los_atom")
