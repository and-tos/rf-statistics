...
def alpha_second_geometry_rf_heatmap(
    second_geometry_name, protein_atom_type, expected_threshold=10, mode="protein"
):
    """
    ...
    plot_df = (
        plot_df.astype(float).sort_index().reindex(sorted(plot_df.columns), axis=1)
    )
    plot = sns.heatmap(
        plot_df,
        cmap="coolwarm",
    ...
    plt.savefig(
        f"2D_a_min_vs_b_min_exp_{expected_threshold}.png", dpi=600, bbox_inches="tight"
    )
    plt.clf()


def favorable_unfavorable_angle_ranges_heatmap(
    output_path,
    input="rf_angle_range_df_with_hitlist.csv",
    geometry="alpha",
):
    """
    ...
    for index, row in df.iterrows():
        if row["type"] != "+" and row["type"] != "-":
            ...
            for alpha in range(0, 180, 10):
                plot_df.loc[row["atom_type"], alpha] = np.nan
        elif row["atom_type"] == "other_ligand":
            for alpha in range(0, 180, 10):
                plot_df.loc[row["atom_type"], alpha] = np.nan
    ...
    try:
        plot = sns.heatmap(
            plot_df, cmap="coolwarm_r", center=1, vmax=2, cbar_kws={"label": "Rf"}
        )
...
def rf_bar_plot(input, title, hue=None, outdir="."):
    """
    ...
    :return: Writes out a png file.
    """
    if type(input) == str or isinstance(input, Path):
    ...
    if len(x) != 0 and len(y) != 0 and len(yerr) != 0:
        if hue is not None:
            ...
            if data.shape[0] > 0:
                ...
                sort_order = (
                    data[data[hue] == "ligand"]
                    .sort_values("rf", ascending=False)["atom_type"]
                    .unique()
                )
                ...
                for index, group in enumerate(groups):
                    ...
                    sub_data.atom_type.cat.set_categories(sort_order)
                    sub_data = sub_data.sort_values(["atom_type"])
                    yerr = (
                        sub_data["rf_high"] - sub_data["rf"],
                        sub_data["rf"] - sub_data["rf_low"],
                    )
                    barwidth = 0.4
                    ...
                    bars.bar(
                        ...
                        label=f"{group} perspective",
                    )
                bars.set_xticks(categories + width / len(groups))
                bars.set_xticklabels(sort_order, rotation=90)
                bars.legend()
        else:
            bars = sns.barplot(
                x=data["atom_type"], y=data["rf"], yerr=yerr, color="skyblue"
            )
            bars.set_xticks(range(len(data)))
...
def _make_heatmap_df(
    input="protein_lookup_alpha.csv", expected_threshold=10, geometry="alpha"
):
    input_df = pd.read_csv(input)
    input_df[f"{geometry}_min"] = input_df[f"{geometry}_min"].astype(float)
    input_df = input_df.sort_values(f"{geometry}_min", ascending=True)

    if geometry in ["alpha", "tau"]:
        columns = range(0, 160, 20)  # range(20, 180, 20)
    elif geometry == "distance":
        columns = np.arange(2.5, 4.0, 0.25)
    else:
        columns = np.arange(0.0, 3.5, 0.5)  # np.arange(0.25, 5, 0.25)
    ...
    for atom_type in input_df["atom_type"].unique():
        ...
        for index, row in df.iterrows():
            geometry_bin = row[f"{geometry}_min"]
            print(geometry_bin)
            # if geometry_bin > 120:
            #     continue
            if row["expected"] >= expected_threshold:
                if row["rf"] == 0:
                    plot_df.loc[atom_type, geometry_bin] = (
                        0.00000001  # for logarithmic colorcoding
                    )
                else:
    ...
    plot_df = plot_df.drop(
        ["acc", "apol", "don", "neg", "other_ligand", "pi", "pos"], errors="ignore"
    )
    return plot_df, mask_df


def plot_protein_geometry_bins(
    input="protein_lookup_alpha.csv",
    ...
    try:
        ...
        plot.figure.axes[-1].set_yticklabels(
            ["0.5", "0.7", "1", "1.5", "2"], minor=False
        )
        ...
        mask_df = mask_df.drop(
            ["acc", "apol", "don", "neg", "other_ligand", "pi", "pos"], errors="ignore"
        )
        plt.rcParams.update({"hatch.color": "grey"})
        plot2 = plt.pcolor(
            np.arange(len(mask_df.columns) + 1),
            np.arange(len(mask_df.index) + 1),
            np.array(mask_df),
            hatch=".....",
            alpha=0.0,
            vmin=vmin,
        ...
        plot.set_xticks(range(len(plot_df.columns)))
        plot.set_xticklabels(plot_df.columns)
        plot.set_ylabel("Protein atom type")
        ...
        outfile = (
            Path(outdir)
            / f"atom_vs_{geometry}_rf_bins_exp_{expected_threshold}_heatmap{extension}.png"
        )
        plt.savefig(
            outfile,
            dpi=300,
...
def plot_rscc_vs_interaction_count(input_data="full_p2cq_pub_oct2019_rf.csv"):
    ...
    for rscc_bin in rscc_bins:
        for count_no in range(0, 8):
            ...
            plot_df.loc[i, f"% entries with n unfavorable contacts"] = (
                pc_entries_with_count_no
            )
    plot = sns.barplot(
        x=plot_df["RSCC"],
        y=plot_df["% entries with n unfavorable contacts"],
        hue=plot_df["unfavorable_contacts_per_entry"],
    )
    plot.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0)
...
def plot_rf_count_vs_expected(input_data):
    ...
    true_rf_bins = [[0.0, 0.25], [0.25, 0.5], [0.5, 0.75], [0.75, 1.0]]
    ...
    boxplot.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0)
    ...
    boxplot.text(
        0.5,
        1.1,
        f"{ligand_atom} {protein_atom}",
        horizontalalignment="center",
        verticalalignment="center",
        transform=boxplot.transAxes,
        fontsize=16,
        weight="bold",
    )
    boxplot.text(
    ...
    sample_numbers = [
        len(binned_data[binned_data["expected"] == bin_]) for bin_ in bins
    ]
    boxplot.set_xticklabels(
        [
            f"Bin: {label} Samples: {sample_numbers[index]}"
            for index, label in enumerate(expected_bins)
        ],
        fontsize=8,
        rotation=90,
    )
    plt.savefig(
        f"{ligand_atom}_{protein_atom}_rf_vs_expected_protein_perspective_boxplot.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.clf()
    ...
    for true_rf_bin in true_rf_bins:
        for expected_bin in expected_bins:
            i += 1
            entries_with_count_no = input_data[
                (input_data["delta_rf_true"] > true_rf_bin[0])
            ...
            plot_df.loc[i, f"delta_rf_true"] = (
                f"{true_rf_bin[0]} < dRf <= {true_rf_bin[1]}"
            )
            plot_df.loc[i, f"expected"] = (
                f"{expected_bin[0]} < expected <= {expected_bin[1]}\nsamples: {total_entries}"
            )
            plot_df.loc[i, f"% dRf"] = pc_entries_with_expected
    plot = sns.barplot(
        x=plot_df["expected"], y=plot_df["% dRf"], hue=plot_df["delta_rf_true"]
    )
    ...
    plt.savefig(
        f"{ligand_atom}_{protein_atom}_rf_vs_expected_protein_perspective.png",
        dpi=300,
        bbox_inches="tight",
    )


def plot_rf_total_vs_pat(
    input_data="full_p2cq_pub_oct2019_rf_extended.gzip",
    ligand_atom_type1="chlorine_aryl",
    ...
    plot_df = plot_df[
        (plot_df["resolution"] <= 2.5)
        & (plot_df["ligand_rscc"] >= 0.8)
        & (plot_df["ligand_avgoccu"] == 1.0)
        & (plot_df["ligand_altcode"] == " ")
    ]
    plot_df = plot_df[
        (plot_df["ligand_atom_type"] == ligand_atom_type1)
        | (plot_df["ligand_atom_type"] == ligand_atom_type2)
    ]

    plot_df_no_uniprot = plot_df[plot_df["uniprot"].isna()].drop_duplicates(
        subset=["ligand_smiles", "project"]
    )
    ...
    print(
        ligand_atom_type2,
        plot_df[plot_df["ligand_atom_type"] == ligand_atom_type2].shape,
    )
    print(
        ligand_atom_type1,
        plot_df[plot_df["ligand_atom_type"] == ligand_atom_type1].shape,
    )
    print(
        ligand_atom_type2,
        plot_df[
            (plot_df["ligand_atom_type"] == ligand_atom_type2)
            & (plot_df["protein_atom_type"] == "O_pi_acc_neg")
        ]["rf_total"].median(),
    )
    print(
        ligand_atom_type1,
    ...
    # generate the plot
    box = sns.boxplot(
        x=plot_df["protein_atom_type"],
        y=plot_df["rf_total"],
        hue=plot_df["ligand_atom_type"],
        showfliers=False,
    )
...
def protein_atom_type_heatmap(expected_threshold=10):
    ...
    protein_atom_types = list(
        pd.read_csv(os.path.join(folders[0], f"ligand_lookup_alpha.csv"))[
            "atom_type"
        ].unique()
    ) + ["metal"]
    geometries = ["alpha", "h"]
    groups = ["acc", "apol", "don", "neg", "other_ligand", "pi", "pos"]
        ...
        for protein_atom_type in protein_atom_types:
            ...
            for mode in ["ligand", "protein"]:
                if (
                    mode == "protein"
                    and geometry == "h"
                    and "pi" not in protein_atom_type
                ):
                    continue
                ...
                for folder in folders:
                    lookup_file = Path(folder) / f"{mode}_lookup_{geometry}.csv"
                    if Path(lookup_file).is_file():
                        lookup_df = pd.read_csv(lookup_file)
                        if lookup_df.shape[0] > 0:
                            lookup_df = lookup_df[
                                lookup_df["atom_type"] == protein_atom_type
                            ]
                ...
                if geometry == "alpha":
                    ...
                    xtick_labels = [
                        int(i) for i in np.linspace(0, 180, 10)
                    ]  # [int(i) for i in np.linspace(0, 180, 19)]
                else:
                ...
                for ligand_atom_type in input_df["ligand_atom_type"].unique():
                    ...
                    for index, row in df.iterrows():
                        ...
                        if row["expected"] >= expected_threshold:
                            if row["rf"] == 0:
                                plot_df.loc[ligand_atom_type, geometry_bin] = (
                                    0.00000001  # for logarithmic colorcoding
                                )
                            else:
                ...
                plot_df = plot_df.sort_index()
                plot_df = plot_df.drop(
                    ["acc", "apol", "don", "neg", "other_ligand", "pi", "pos"],
                    errors="ignore",
                )
                ...
                plot = sns.heatmap(
                    ...
                    vmin=vmin,
                    vmax=vmax,
                    cbar_kws={"label": "Rf", "ticks": [0.5, 0.7, 1, 1.5, 2]},
                    ax=ax,
                )
                plot.figure.axes[-1].set_yticklabels(
                    ["0.5", "0.7", "1", "1.5", "2"], minor=False
                ...
                plt.savefig(
                    f"{protein_atom_type}_{geometry}_heatmap_{mode}_perspective.png",
                    dpi=dpi,
                    bbox_inches="tight",
                )
                plt.clf()
...
def main():
    ...
    plot_protein_geometry_bins(
        input="ligand_lookup_distance.csv",
        expected_threshold=10,
        geometry="distance",
        mode="ligand",
        title="",
