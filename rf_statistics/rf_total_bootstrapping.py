from rf_statistics import los_analysis
import pandas as pd
import numpy as np
...
class RfTotalBoots(object):
    ...
    def angle_dependency(self, input_path="", mode="protein", protein_atom_types=None):
        ...
        pseudo_rfs = pseudo_rfs.assign(
            SMARTS=self.smarts, SMARTS_index=self.smarts_index
        )
        return pseudo_rfs

    def distance_dependency(
        self, second_geometry_name, bin_size=0.5, input_path="", mode="protein"
    ):
        """
        Wrapper that calculates Rf values for attractive and repulsive ranges. Assign unique ligand, protein and binding
         site counts.
    ...
    def return_rf_total(
        ...
        rf_protein_h = protein_h_df[protein_h_df["h_min"] == protein_h_min][
            "rf"
        ].to_numpy()[0]

        rf_protein_alpha = protein_alpha_df[
            protein_alpha_df["alpha_min"] == protein_alpha_min
        ]["rf"].to_numpy()[0]

        rf_ligand_alpha = ligand_alpha_df[
            (ligand_alpha_df["alpha_min"] == ligand_alpha_min)
        ...
        ligand_alpha_expected = ligand_alpha_df[
            (ligand_alpha_df["alpha_min"] == ligand_alpha_min)
            & (ligand_alpha_df["atom_type"] == protein_atom_type)
        ]["expected"].to_numpy()[0]
        protein_alpha_expected = protein_alpha_df[
            protein_alpha_df["alpha_min"] == protein_alpha_min
        ]["expected"].to_numpy()[0]
        protein_h_expected = protein_h_df[protein_h_df["h_min"] == protein_h_min][
            "expected"
        ].to_numpy()[0]
        return (
            rf_total,
            ligand_alpha_expected,
            protein_alpha_expected,
            protein_h_expected,
        )
    ...
    def rf_total_bootstrap(self, confidence_level=0.05):
        ...
        ligand_alpha_pseudo_rfs = self.angle_dependency(
            input_path="../query_atom",
            mode="ligand",
        ...
        for protein_alpha_min in protein_alpha_pseudo_rfs["alpha_min"].unique():
            for protein_h_min in protein_h_pseudo_rfs["h_min"].unique():
                for ligand_alpha_min in ligand_alpha_pseudo_rfs["alpha_min"].unique():
                    df = pd.DataFrame()
                    df["pseudo_rf_protein_alpha"] = protein_alpha_pseudo_rfs[
                        protein_alpha_pseudo_rfs["alpha_min"] == protein_alpha_min
                    ]["pseudo_rf"]
                    df["pseudo_rf_protein_h"] = protein_h_pseudo_rfs[
                        protein_h_pseudo_rfs["h_min"] == protein_h_min
                    ]["pseudo_rf"]
                    df["pseudo_rf_protein"] = df.apply(
                        lambda x: gmean(
                            [x["pseudo_rf_protein_alpha"], x["pseudo_rf_protein_h"]]
                        ),
                        axis=1,
                    )

                    df["pseudo_rf_ligand_alpha"] = ligand_alpha_pseudo_rfs[
                        ligand_alpha_pseudo_rfs["alpha_min"] == ligand_alpha_min
                    ]["pseudo_rf"]
                    df["pseudo_rf_ligand_h"] = 1

                    df["pseudo_rf_ligand"] = df.apply(
                        lambda x: gmean(
                            [x["pseudo_rf_ligand_alpha"], x["pseudo_rf_ligand_h"]]
                        ),
                        axis=1,
                    )

                    df["rf_total"] = df.apply(
                        lambda x: gmean(
                            [x["pseudo_rf_protein"], x["pseudo_rf_ligand"]]
                        ),
                        axis=1,
                    )
                    rf_sorted = (
                    ...
                    rf_tot_low = rf_sorted[
                        int((confidence_level / 2.0) * len(rf_sorted))
                    ]
                    rf_tot_high = rf_sorted[
                        int((1 - confidence_level / 2.0) * len(rf_sorted))
                    ]
                    (
                        rf_total,
                        ligand_alpha_expected,
...
class Comparator(object):
    ...
    def propagation_vs_boots(self):
        bootstrapping_df = self.bootstrapping_df[
            (self.bootstrapping_df["ligand_alpha_expected"] > 10)
            & (self.bootstrapping_df["protein_alpha_expected"] > 10)
            & (self.bootstrapping_df["protein_h_expected"] > 10)
        ]
        database_rf = self.database_rf[
            (self.database_rf["SMARTS"] == "[*]:,=[#6][Cl]")
            & (self.database_rf["protein_atom_type"] == "O_pi_acc")
        ]
        database_rf = database_rf.astype(
            {"protein_h": float, "protein_alphas": float, "ligand_alphas": float}
        )
        ...
        for index, row in bootstrapping_df.iterrows():
            try:
                ...
                propagated_error = database_rf[
                    (database_rf["ligand_alphas"] >= ligand_alpha_min)
                ...
                propagated_rf_total = database_rf[
                    (database_rf["ligand_alphas"] >= ligand_alpha_min)
                    & (database_rf["ligand_alphas"] <= ligand_alpha_min + 20)
                    & (database_rf["protein_alphas"] >= protein_alpha_min)
                    & (database_rf["protein_alphas"] <= protein_alpha_min + 20)
        ...
        plot2 = sns.lineplot(x=[0.0, 2], y=[0, 2])
